...
class DatabaseManager:
    """Handles all SQLite database connections and operations, including accounting."""
...
class OrderEntryWidget(QWidget):
    ...
    def _setup_autocompleters(self):
        ...
        # Supplier Completer
        supplier_names = self.db.get_supplier_names()
        self.supplier_completer_model = QStringListModel(supplier_names)
        self.supplier_completer = QCompleter(self.supplier_completer_model)
        self.supplier_completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        self.supplier_completer.setFilterMode(Qt.MatchFlag.MatchContains)
    ...
    def _setup_ui(self):
        ...
        line_edit_style = "QLineEdit, QDateEdit {padding: 5px 8px; min-height: 20px; font-size: 9pt;}"
        
        # 1. Header and Supplier Info (UNCHANGED)
        ...
        self.supplier_input = QLineEdit(); self.supplier_input.setStyleSheet(line_edit_style)
        ...
        header_layout.addWidget(create_dense_label("Supplier Name:"), 0, 0); header_layout.addWidget(self.supplier_input, 0, 1)
        header_layout.addWidget(create_dense_label("Invoice No:"), 1, 0); header_layout.addWidget(self.invoice_number_input, 1, 1)
        header_layout.addWidget(create_dense_label("Invoice Date:"), 2, 0); header_layout.addWidget(self.invoice_date_input, 2, 1)
        
        main_layout.addWidget(header_group)
        
        # 2. Add Item Section (UPDATED with Selling Price)
        ...
        self.pack_selling_input = QLineEdit() # <-- NEW INPUT
        self.pack_selling_input.setValidator(QDoubleValidator(0.00, 99999.99, 2)) # <-- NEW VALIDATOR
        ...
        self.pack_selling_input.setStyleSheet(line_edit_style) # <-- NEW STYLE
        ...
        item_layout.addWidget(create_dense_label("Pack Selling ($):"), 3, 0); item_layout.addWidget(self.pack_selling_input, 3, 1) # <-- NEW ROW
        ...
        item_layout.addWidget(self.add_item_button, 4, 2, 1, 2) # <-- ADJUSTED ROW INDEX
        item_layout.addWidget(self.product_status_label, 5, 0, 1, 4) # <-- ADJUSTED ROW INDEX
    ...
    def add_item_to_list(self):
        ...
        # 2. Numeric Input Validation (UPDATED)
        try:
            qty = float(self.qty_input.text())
            cost = float(self.pack_cost_input.text())
            selling = float(self.pack_selling_input.text()) # <-- NEW
            if qty <= 0 or cost <= 0 or selling <= 0: raise ValueError # <-- Validate Selling Price
        except ValueError:
        ...
        # 3. Required Field Check (UNCHANGED)
        ...
        self.pending_order_items.append({
            ...
            'pack_quantity': qty,
            'pack_cost': cost,
            'pack_selling_price': selling, # <-- NEW
        ...
        self.pack_cost_input.clear()
        self.pack_selling_input.clear() # <-- NEW
        self.product_search_input.setFocus()
    ...
    def finalize_order(self):
        ...
        if not supplier_name:
            QMessageBox.warning(self, "Error", "Please enter a Supplier Name before finalizing.")
            self.supplier_input.setFocus()
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # Add supplier name if new
            self.db.add_supplier_name(supplier_name)
...
class PurchasingWidget(QTabWidget):
    """Container for Purchase Order Entry and Supplier Payments."""
    def __init__(self, db, user_id):
        super().__init__()
        self.db = db
        self.user_id = user_id
        ...
        # REPLACED: Use the new POManagerWidget
        ...
        self.addTab(self.po_manager_widget, "2. Manage Purchase Orders & Payments") # RENAMED TAB
        
        self.currentChanged.connect(self._handle_tab_change)

    def _handle_tab_change(self, index):
        # Update the condition to match the new tab name
        if self.tabText(index) == "2. Manage Purchase Orders & Payments":
            self.po_manager_widget.load_pending_invoices() # Calls the updated loading method
...
class AddProductDialog(QDialog):
    ...
    def __init__(self, db, parent=None):
        ...
        self.supplier_names = self.db.get_supplier_names() 
        self.company_names = self.db.get_company_names() 
        
        self._setup_ui()
        self._setup_supplier_completer() 
        self._setup_company_completer() 
        self.class_combo.currentIndexChanged.connect(self.update_units_per_pack_state)
        
    # --- Completer methods ---
    def _setup_supplier_completer(self):
        """Sets up QCompleter for the supplier input."""
        self.supplier_model = QStringListModel(self.supplier_names)
        self.supplier_completer = QCompleter(self.supplier_model)
        self.supplier_completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        self.supplier_completer.setFilterMode(Qt.MatchFlag.MatchContains)
        self.supplier_input.setCompleter(self.supplier_completer)
    ...
    def _setup_ui(self):
        ...
        form_layout.addWidget(QLabel("Supplier (Default/Optional):"), 4, 0); form_layout.addWidget(self.supplier_input, 4, 1)
        form_layout.addWidget(QLabel("Reorder Packs:"), 5, 0); form_layout.addWidget(self.reorder_input, 5, 1)
    ...
    def save_data(self):
        ...
        # Add optional names to their respective master lists for future autocompletion
        if supplier_name: self.db.add_supplier_name(supplier_name)
        if company_name: self.db.add_company_name(company_name) 

        try:
            with self.db.conn:  
                # MODIFIED QUERY: Removed selling_price and cost_price columns
                query = """INSERT INTO products (name, class_id, units_per_pack, unit_type, company, supplier, formulation, reorder_point) 
                                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)"""
                self.db.cursor.execute(query, (name, class_id, units_per_pack, class_name, company_name, supplier_name, self.formulation_input.text(), reorder))
            ...
            if hasattr(main_window, 'user_id'):
                ...
                self.db.log_action(
                    ...
                    'PRODUCT_CREATE',
                    product_id,
                    f"Name: {name}. Master record created. Prices to be set in Purchasing."
                )
            
            QMessageBox.information(self, "Success", f"Product '{name}' saved successfully. **Prices must be set in Purchasing/Receive Stock.**")
            self.accept()
...
class ProductDetailDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        master_layout.addWidget(QLabel("Supplier (Default):"), 1, 2); master_layout.addWidget(self.supplier_input, 1, 3) 
        master_layout.addWidget(QLabel("Company (Default):"), 2, 2); master_layout.addWidget(self.company_input, 2, 3) 
    ...
    def load_product_data(self):
        # MODIFIED QUERY: Removed static price fields
    ...
    def save_master_data(self):
        ...
        new_supplier = self.supplier_input.text().strip()
        ...
        if new_supplier: self.db.add_supplier_name(new_supplier)
        if new_company: self.db.add_company_name(new_company)
            
        try:
            with self.db.conn:
                # MODIFIED QUERY: Removed price fields
                query = """UPDATE products SET name=?, class_id=?, units_per_pack=?, unit_type=?, reorder_point=?, supplier=?, company=? WHERE product_id=?"""
                self.db.cursor.execute(query, (new_name, new_class_id, new_upp, new_unit_type, new_reorder, new_supplier, new_company, self.product_id))
            
            # NEW: Log the Product Master Update event (Modified details)
            main_window = self.parent().parent()
            user_info = self.db.cursor.execute("SELECT username FROM users WHERE id=?", (main_window.user_id,)).fetchone()
            username = user_info[0] if user_info else 'System'
            self.db.log_action(
                ...
                'PRODUCT_UPDATE',
                self.product_id,
                f"Updated master data for {self.original_product_name} -> {new_name}."
            )
...
class InventoryWidget(QWidget):
    ...
    def _create_permanent_elements(self):
        ...
        self.search_bar = QLineEdit()
        self.search_bar.setPlaceholderText("Search product name, unit type, or supplier...")
        ...
        self.product_master_table.setColumnCount(8) 
        self.product_master_table.setHorizontalHeaderLabels(["ID", "Name", "Class", "Total Stock", "Units/Pack", "Unit Type", "Reorder", "Supplier"])
        ...
        # Controls and search take minimal space (stretch 0)
        ...
        # Remove the final QWidget.addStretch(1) for cleaner layout handling.
    ...
    def load_product_master(self):
        ...
        # MODIFIED: Removed p.selling_price and p.cost_price from SELECT
        base_query = """
        SELECT  
            p.product_id, 
            p.name, 
        ...
        # 2. Build the WHERE clause (UNCHANGED LOGIC)
        where_clauses = []
        params = []
        
        if search_text:
            search_param = f'%{search_text}%'
            where_clauses.append("(p.name LIKE ? OR c.name LIKE ? OR p.unit_type LIKE ? OR p.supplier LIKE ? OR p.company LIKE ?)")
        ...
        for row_num, product in enumerate(products):
            # UPDATED unpacking: Removed static prices (sp, cp). company is discarded from the list view.
            product_id, name, class_name, total_stock_packs, units_per_pack, unit_type, reorder_point, supplier, company = product
            ...
            # UPDATED data list: Removed static prices and Company
            data = [
                str(product_id), name, class_name, str(total_stock_packs), str(units_per_pack), 
                unit_type, str(reorder_point), supplier
            ]
...
# ==============================================================================
# 6. ALERT WIDGETS (UNCHANGED)
# ==============================================================================
# ==============================================================================
# NEW: DIALOG FOR SELECTING SALE ITEMS TO RETURN
# ==============================================================================

# ==============================================================================
# CORRECTED: DIALOG FOR SELECTING SALE ITEMS TO RETURN (V3)
# ==============================================================================
...
# ==============================================================================
# MODIFIED: CUSTOMER RETURNS WIDGET
# ==============================================================================

# ==============================================================================
# MODIFIED: CUSTOMER RETURNS WIDGET (WITH CONFIRMATION MESSAGE)
# ==============================================================================
# ==============================================================================
# NEW: DIALOG FOR VIEWING SINGLE RETURN TRANSACTION DETAILS
# ==============================================================================
