...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QSpinBox, QFileDialog, QScrollArea, QFrame, QSizePolicy,
    QDoubleSpinBox
)
from PyQt6.QtCore import Qt, QDate, QStringListModel
from PyQt6.QtGui import QColor, QFont, QDoubleValidator, QIntValidator

# Imports for ReportLab PDF generation (REQUIRED)
from reportlab.lib.pagesizes import A4
from reportlab.lib import colors
from reportlab.lib.units import mm
from reportlab.pdfgen import canvas
# Note: You can remove the unused ReportLab imports (SimpleDocTemplate, Table, TableStyle, Paragraph, getSampleStyleSheet) 
# as your final implementation only uses canvas directly. However, they are kept here for historical compatibility 
# as they were part of the original project structure.
from reportlab.lib.pagesizes import A4
from reportlab.lib import colors
from reportlab.lib.units import mm
from reportlab.pdfgen import canvas
...
# ==============================================================================
# 1. DATABASE MANAGER (THE DATA ENGINE) - COMPLETE REPLACEMENT (MRP ONLY FOCUS)
# ==============================================================================
...
# ==============================================================================
# 1. DATABASE MANAGER (THE DATA ENGINE) - MODIFIED: COST/PROFIT REMOVED
# ==============================================================================

class DatabaseManager:
    """
    Handles all SQLite database connections and operations.
    MODIFIED: Cost Price, COGS, and Profit tracking removed.
    """
    ...
    def create_tables(self):
        ...
        # 1. Master Tables (UNCHANGED STRUCTURE)
        ...
        # 3. Products Master Table (UNCHANGED STRUCTURE, but cost assumptions gone)
        self.cursor.execute("""
        ...
        # 4. PURCHASE ORDERS (UNCHANGED STRUCTURE)
        self.cursor.execute("""
        ...
        # 7. Stock Batches Table (MODIFIED: pack_cost_price REMOVED)
        self.cursor.execute("""
        ...
        # 8-12. Users, Sales, Sale_Items, Returns, Audit_Log (UNCHANGED STRUCTURE FOR SALES HEADER)
    ...
    # --- PURCHASING & PAYABLES METHODS (MODIFIED: COST REMOVED) ---
    
    # MODIFIED: Removed pack_cost_price from arguments and database query
    def create_purchase_order(self, supplier_name, final_bill_value, items):
        """Creates a new PO and related items using a FIXED final_bill_value (Bill Value)."""
        current_date = datetime.date.today().strftime('%Y-%m-%d')
        
        try:
            with self.conn:
                # 1. Create Purchase Order Header using the external final_bill_value
                self.cursor.execute(
                    """INSERT INTO purchase_orders (supplier_name, order_date, total_invoice_amount, status)
                        VALUES (?, ?, ?, ?)""",
                    (supplier_name, current_date, final_bill_value, 'Received/Unpaid')
                )
                order_id = self.cursor.lastrowid
                
                # 2. Insert Purchase Items and Stock Batches (COST REMOVED)
                for item in items:
                    # 2a. Insert into purchase_items (PACK COST REMOVED)
                    self.cursor.execute(
                        """INSERT INTO purchase_items (order_id, product_id, batch_number, pack_quantity_ordered, date_received)
                            VALUES (?, ?, ?, ?, ?)""",
                        (order_id, item['product_id'], item['batch_number'], item['pack_quantity'], current_date)
                    )
                    p_item_id = self.cursor.lastrowid
                    
                    # 2b. Insert into stock_batches (PACK COST REMOVED)
                    self.cursor.execute(
                        """INSERT INTO stock_batches (product_id, purchase_item_id, batch_number, stock_quantity, expiry_date, date_received, pack_selling_price)
                            VALUES (?, ?, ?, ?, ?, ?, ?)""",
                        (item['product_id'], p_item_id, item['batch_number'], item['pack_quantity'], item['expiry_date'], current_date, item['pack_selling_price'])
                    )
    ...
    def record_supplier_payment(self, order_id, amount_paid, payment_method, transaction_ref, notes, user_id):
        """Records a payment against a specific purchase order/invoice. (UNCHANGED LOGIC)"""
        ...
        try:
            with self.conn:
                ...
                # Simple status update: if net pending is zero, mark as paid. (UNCHANGED)
    ...
    def get_supplier_pending_invoices(self):
        """Retrieves invoices that are not fully paid or have open balances. (UNCHANGED LOGIC)"""
        query = """
        SELECT  
            T1.order_id,
            T1.supplier_name,
        ...
        # Integrate returns credit calculation before final presentation (UNCHANGED)
        results = []
    ...
    def get_order_pending_amount(self, order_id):
        """Calculates the net pending amount for a single order. (UNCHANGED LOGIC)"""
        ...
        # Sum returns credit (UNCHANGED, but hardcoded to 0.0 in its definition below)
        return_credit = self.get_order_returns_credit(order_id)
        
        return max(0.0, total_invoice_amount - (total_paid + return_credit))

    # MODIFIED: Total Refund Value removed from Returns table DDL, thus this method needs a fallback if called.
    def get_order_returns_credit(self, order_id):
        """Calculates the total return credit applied to a specific order (now returns 0.0)."""
        return 0.0  # Hardcoded to 0.0 as cost/credit tracking is removed for simplicity
    
    def get_purchase_order_details(self, order_id):
        """Retrieves all header, line items, and payment details for a specific PO. (MODIFIED: COST REMOVED)"""
        ...
        # 2. Fetch Line Items (MODIFIED: pack_cost_price REMOVED)
        query_items = """
        SELECT T1.p_item_id, T2.name, T1.batch_number, T1.pack_quantity_ordered, T2.units_per_pack, T2.unit_type
        ...
        # 3. Fetch Payment/Return Summary (MODIFIED: Returns credit is 0.0)
        self.cursor.execute("SELECT COALESCE(SUM(amount_paid), 0.0) FROM supplier_payments WHERE order_id = ?", (order_id,))
        total_payments = self.cursor.fetchone()[0] or 0.0
        total_credit = 0.0 # Hardcoded to 0.0 since we removed credit value from returns
    ...
    def soft_void_purchase_order(self, order_id, user_id):
        ...
        Only works if no PAYMENTS have been made against the PO. (MODIFIED: Removed credit check, since it's gone)
        """
        if not self.conn: return "DB connection inactive."
        
        # 1. Check if the order has been paid
        ...
        # No more check for credit since it's removed.

        try:
            with self.conn:
                # 2. Fetch Items and Batches to Revert
                # Removed pack_cost_price from SELECT
                ...
                for p_item_id, product_id, packs_ordered in items:
                    ...
                    if batch_data:
                        ...
                        # Deduct the ordered quantity from stock (must account for partial sales) (UNCHANGED)
                        ...
                        # Remove purchase_item record (UNCHANGED)
                        self.cursor.execute("DELETE FROM purchase_items WHERE p_item_id = ?", (p_item_id,))
                        
                        # Remove stock_batch record if quantity is now 0 and no sales/returns history exists. (UNCHANGED)
                ...
                # 3. Update PO Header Status & Amount (UNCHANGED)
                ...
                # 4. Log the Void Action (UNCHANGED)
    ...
    # --- SALES & RETURN OVERRIDES (MODIFIED: COST/REFUND VALUE REMOVED) ---
    
    # MODIFIED: cost_price and total_refund_value removed from arguments and query
    def record_return_and_deduct_stock(self, product_id, batch_id, quantity_returned_packs, supplier_name, reason):
        """
        Inserts return record (Supplier Return), links to a purchase order if possible, and updates stock_batches.
        MODIFIED: Cost Price and Refund Value removed.
        """
        if not self.conn: return False
        current_date = datetime.date.today().strftime('%Y-%m-%d')

        # Try to find the original purchase order ID via the batch (UNCHANGED)
        order_id = None
        self.cursor.execute("""
        ...
        try:
            with self.conn:
                # 1. Insert into Returns table (COST & REFUND REMOVED)
                self.cursor.execute(
                    """INSERT INTO returns (return_date, product_id, batch_id, order_id, quantity_returned_packs, supplier_name, reason)
                    VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (current_date, product_id, batch_id, order_id, quantity_returned_packs, supplier_name, reason)
                )

                # 2. Update Stock Batches (Deduct the returned quantity) (UNCHANGED)
                self.cursor.execute(
    ...
    # MODIFIED: Implements check to prevent returning more than net remaining
    def record_sales_return_and_restore_stock(self, sale_id, product_name, qty_returned_base_units, reason, user_id):
        ...
        Includes a critical check to ensure the return quantity does not exceed the net sold quantity (original - previous returns).
        """
        ...
        try:
            with self.conn:
                ...
                self.cursor.execute("""
                    SELECT  
                        si.id, si.product_id, si.batch_id, si.quantity_sold, si.unit_price,
                ...
                si_id, product_id, batch_id, quantity_sold_units, unit_price_at_sale, units_per_pack, unit_type = sale_item_data

                packs_per_unit = units_per_pack if units_per_pack > 0 else 1
                
                # *** CRITICAL FIX: Calculate Net Available for Return AGAINST THIS SALE_ID ***
                # We query the 'returns' table explicitly using the supplier_name audit trail
                self.cursor.execute("""
                    SELECT COALESCE(SUM(r.quantity_returned_packs), 0.0) 
                    FROM returns r
                    WHERE r.supplier_name = ? 
                    AND EXISTS (
                        SELECT 1 FROM sale_items si_ref
                        WHERE si_ref.sale_id = ? 
                        AND si_ref.product_id = ?
                        AND si_ref.batch_id = r.batch_id
                    )
                """, (f'Customer Return from Invoice {sale_id}', sale_id, product_id))
                ...
                net_units_available_to_return = max(0.0, quantity_sold_units - units_previously_returned)
                
                # 2. Enforce the Net Limit
                if qty_returned_base_units > net_units_available_to_return + 0.001:
                    return None, (
                        f"Return quantity ({qty_returned_base_units:.2f} units) exceeds the net quantity "
                        f"available for return ({net_units_available_to_return:.2f} units) for this sale "
                        f"(original sold minus prior returns)."
                    )
                # *** END CRITICAL FIX ***

                # 2. Calculate Refund Value (BASED ON ORIGINAL SELLING PRICE)
                refund_value = round(qty_returned_base_units * unit_price_at_sale, 2)

                # 3. Calculate Packs to restore
                packs_to_restore = round(qty_returned_base_units / packs_per_unit, 3)

                # 4. Insert into Returns Table (Note: supplier_name now explicitly holds the Sale ID reference)
                self.cursor.execute(
                    """INSERT INTO returns (return_date, product_id, batch_id, order_id, quantity_returned_packs, supplier_name, reason)
                        VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (datetime.date.today().strftime('%Y-%m-%d'), product_id, batch_id, None, packs_to_restore, f'Customer Return from Invoice {sale_id}', reason)
                )
                return_id = self.cursor.lastrowid
                
                # 5. Restore Stock in the original batch (UNCHANGED)
                self.cursor.execute(
                    "UPDATE stock_batches SET stock_quantity = ROUND(stock_quantity + ?, 3) WHERE batch_id = ?",
                    (packs_to_restore, batch_id)
                )
                
                # 6. Log the Action (UNCHANGED)
                user_info = self.cursor.execute("SELECT username FROM users WHERE id=?", (user_id,)).fetchone()
                username = user_info[0] if user_info else 'System'
                self.log_action(
                    user_id,
    ...
    # MODIFIED: Removed all cost/profit calculations.
    def soft_void_sale_transaction(self, sale_id, user_id):
        ...
        and setting the sale status to 'Voided'.
        MODIFIED: Removed all cost/profit logic from logging/calculations.
        """
        if not self.conn: return "DB connection inactive."

        try:
            with self.conn:
                # 1. Check current status (UNCHANGED)
                ...
                # 2. FETCH all items sold (MODIFIED: Removed cost_price_at_sale from SELECT)
                self.cursor.execute("""
                    SELECT  
                        si.product_id, si.batch_id, si.quantity_sold, si.unit_price, 
                ...
                if not sale_items:
                    # If there are no items, just mark the header as voided. (UNCHANGED)
                ...
                for item in sale_items:
                    product_id, batch_id, qty_sold_units, unit_price_at_sale, units_per_pack, product_name, expiry_date = item
                    
                    packs_per_unit = units_per_pack if units_per_pack > 0 else 1

                    # --- Calculate net quantity to restore (CRITICAL FIX - UNCHANGED LOGIC) ---
                    ...
                    # Calculate the net financial impact for the audit log (BASED ON SELLING PRICE)
                    refund_value_net = round(qty_to_restore_units * unit_price_at_sale, 2)
                    total_reversal_amount += refund_value_net

                    # a. Insert full reversal into Returns table (COST/REFUND REMOVED)
                    self.cursor.execute(
                        """INSERT INTO returns (return_date, product_id, batch_id, order_id, quantity_returned_packs, supplier_name, reason)
                            VALUES (?, ?, ?, ?, ?, ?, ?)""",
                        (today, product_id, batch_id, None, packs_to_restore, f'VOID SALE {sale_id}', 'Full Sale Void/Reversal (Net of Customer Returns)')
                    )
                    
                    # b. Restore Stock in the original batch (UNCHANGED)
                    self.cursor.execute(
                        "UPDATE stock_batches SET stock_quantity = ROUND(stock_quantity + ?, 3) WHERE batch_id = ?",
                        (packs_to_restore, batch_id)
                    )
                ...
                # 4. Update the original sale status (Soft Void) (UNCHANGED)
                self.cursor.execute("UPDATE sales SET status = 'Voided', total_amount = 0.0, discount = 0.0 WHERE id = ?", (sale_id,))

                # 5. Log the reversal (Void) action (UNCHANGED)
                user_info = self.cursor.execute("SELECT username FROM users WHERE id=?", (user_id,)).fetchone()
                username = user_info[0] if user_info else 'System'
                self.log_action(
                    user_id,
    ...
    def get_sale_items_for_display(self, sale_id):
        """
        Retrieves all line items and necessary product/batch info for a specific sale.
        Calculates remaining returnable quantity based on original sale amount minus previous customer returns.
        """
        if not self.conn: return None
        try:
            query = """
            ...
            formatted_results = []
            for name, units_per_pack, unit_type, qty_sold, unit_price, batch_number, expiry_date in results:
                
                packs_per_unit = units_per_pack if units_per_pack > 0 else 1
                
                # FIX: We must calculate returns ONLY for this specific SALE_ID.
                # The logic must find all return records that occurred in a batch 
                # that was originally sold as part of THIS sale.
                
                # Check how much quantity (in packs) has already been processed in customer returns for this sale/product combo
                self.cursor.execute("""
                    SELECT COALESCE(SUM(r.quantity_returned_packs), 0.0) 
                    FROM returns r
                    -- We only care about returns where the supplier_name is the audit trail
                    -- identifying the original sale ID.
                    WHERE r.supplier_name = ? 
                    AND EXISTS (
                        SELECT 1 FROM sale_items si_ref
                        JOIN products p_ref ON si_ref.product_id = p_ref.product_id
                        WHERE si_ref.sale_id = ? 
                        AND p_ref.name = ? 
                        AND si_ref.batch_id = r.batch_id
                    )
                """, (f'Customer Return from Invoice {sale_id}', sale_id, name))
                total_packs_returned_for_this_sale = self.cursor.fetchone()[0] or 0.0
                
                units_returned = total_packs_returned_for_this_sale * packs_per_unit
                qty_remaining_for_return = max(0.0, qty_sold - units_returned)
                ...
                formatted_results.append({
                    ...
                    'total_price': round(qty_sold * unit_price, 2),
                    'batch_number': batch_number or 'N/A',
                    'expiry_date': exp_display,
                    'qty_remaining_for_return': qty_remaining_for_return
                })
                
            return formatted_results
        except sqlite3.Error as e:
            print(f"❌ Error fetching sale items for display: {e}")
            return None
    
    # --- NEW DAILY SALES METHOD (MODIFIED: Profit/COGS REMOVED) ---
    def get_daily_sales_revenue(self, target_date):
        """Fetches finalized sales data for a specific day, returning only revenue (no profit)."""
        if not self.conn: return []
        ...
        # 2. Iterate and format data (NO PROFIT CALCULATION)
        for sale_id, datetime_str, revenue, discount, patient_name, doctor_ref in sales_headers:
            ...
            report_results.append({
                ...
                'revenue': revenue,
                'discount': discount,
                # 'profit' and 'cogs' are excluded
            })
            
        return report_results
    
    # **>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>**
    # ** NEW METHOD: Inventory Valuation using Selling Price (SP) **
    # **<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<**
    def get_inventory_valuation_sp(self):
        """
        Calculates the total selling value (MRP/SP) of all current, non-expired stock,
        and provides detailed breakdown per product.
        """
        if not self.conn: return {'total_value': 0.0, 'details': []}

        # Query to calculate the value of remaining stock based on its pack_selling_price
        query = """
        SELECT
            p.product_id,
            ...
            p.company,
            SUM(T1.stock_quantity) AS TotalPacks,
            AVG(T1.pack_selling_price) AS AvgPackSellingPrice,
            SUM(T1.stock_quantity * T1.pack_selling_price) AS TotalSellingValue
        FROM stock_batches T1
        JOIN products p ON T1.product_id = p.product_id
        LEFT JOIN classes c ON p.class_id = c.class_id
        WHERE T1.stock_quantity > 0 AND T1.expiry_date >= STRFTIME('%Y-%m', 'now')
        GROUP BY p.product_id
        HAVING TotalPacks > 0.001
        ORDER BY p.name ASC;
        """
        self.cursor.execute(query)
        results = self.cursor.fetchall()

        # Row structure: ID, Name, Class, Company, TotalPacks, AvgSP, TotalSPValue
        total_value = sum(row[6] for row in results)
        
        details = []
        for row in results:
            product_id, name, class_name, company, total_packs, avg_sp, total_value_sp = row
            details.append((
                name, 
                class_name or 'N/A', 
                company or 'N/A', 
    ...
    # --- INVENTORY/SETUP UTILITIES (MODIFIED: Inventory Value updated) ---

    def get_supplier_names(self):
        """Retrieves all unique supplier names (combining supplier and purchase_orders). (UNCHANGED)"""
        if not self.conn: return []
    ...
    def add_initial_data(self):
        """Creates the bare minimum data (Admin user) and inserts default classes. (UNCHANGED)"""
    ...
    def get_expiry_alerts(self):
        """Retrieves batches that are expired, or expiring in the next 60 days. (UNCHANGED LOGIC)"""
    ...
    def get_expired_batches(self):
        """Retrieves ONLY batches that have an expiry date less than today (YYYY-MM). (UNCHANGED LOGIC)"""
        if not self.conn: return []
    ...
    def get_returnable_batches(self):
        """Retrieves active batches that are expired, or expiring in the next 90 days. (MODIFIED: COST REMOVED)"""
        if not self.conn: return []
        
        # MODIFIED: Removed pack_cost_price from SELECT
        query = """
        SELECT  
            T2.batch_id, T1.product_id, T1.name, T1.supplier, T2.date_received,  
            T2.expiry_date, T2.stock_quantity, T1.units_per_pack, T1.unit_type, T2.batch_number
        FROM products AS T1
        JOIN stock_batches AS T2 ON T1.product_id = T2.product_id
        ...
        """
        self.cursor.execute(query)
        # Note: The code relying on this data must also be updated to expect one less column (no pack_cost_price).
        # Original: 11 columns (0-10). New: 10 columns (0-9).
        return self.cursor.fetchall()
        
    def add_supplier_name(self, name):
        """Inserts a new supplier name if it doesn't exist. (UNCHANGED)"""
        if not self.conn: return False
        try:
            with self.conn:
    ...
    def get_company_names(self):
        """Retrieves all unique company names. (UNCHANGED)"""
        if not self.conn: return []
        self.cursor.execute("SELECT name FROM companies ORDER BY name ASC")
        return [row[0] for row in self.cursor.fetchall()]

    def add_company_name(self, name):
        """Inserts a new company name if it doesn't exist. (UNCHANGED)"""
        if not self.conn: return False
        try:
            with self.conn:
    ...
    def get_product_stock_by_name(self, product_name):
        """Retrieves total active stock (packs) for a given product name. (UNCHANGED)"""
        if not self.conn: return None
    ...
    def get_dashboard_data(self):
        """Fetches consolidated data for the dashboard. MODIFIED: Inventory Value now uses SP. (UNCHANGED LOGIC for other parts)"""
        if not self.conn: return {
        ...
        # 1. Total Inventory Selling Value (MODIFIED to use the new SP method)
        total_inventory_value = self.get_inventory_valuation_sp()['total_value']
        
        # 2. Total Low Stock Items (Below Reorder Point) (UNCHANGED)
        self.cursor.execute("""
        ...
        # 3. Sales/Revenue for the Last 7 Days (Excluding Voided Sales) (UNCHANGED)
        ...
        # 4. Expiry Alerts (Counts only) (UNCHANGED)
    ...
    def perform_daily_backup(self):
        """Creates a timestamped backup file in a 'backups' directory and cleans up old files. (UNCHANGED)"""
    ...
    def _cleanup_old_backups(self, backup_dir):
        """Implements a simple retention policy: keep only the last 7 backups."""

        all_backups = []

        # 1. Collect all backup files with their full path and modification time
        for filename in os.listdir(backup_dir):
            if filename.endswith(".db") and filename.startswith("pharmacy_pos_backup_"):
                full_path = os.path.join(backup_dir, filename)
                try:
                    # Use modification time for the definitive age/order
                    mod_time = os.path.getmtime(full_path)
                    all_backups.append({'filename': filename, 'path': full_path, 'time': mod_time})
                except OSError:
                    # Skip files we cannot access
                    continue

        # 2. Sort the backups by time (newest first)
        # We sort by 'time' in descending order (reverse=True)
        all_backups.sort(key=lambda x: x['time'], reverse=True)

        # 3. Determine which files to keep (the top 7)
        files_to_keep = {backup['filename'] for backup in all_backups[:7]}

        # 4. Delete files not in the 'keep' set
        deleted_count = 0
        ...
        for backup in all_backups:
            if backup['filename'] not in files_to_keep:
                try:
                    ...
                    # Pass silently if file deletion fails
                    pass  
    ...
    def get_doctor_names(self):
        """Retrieves all unique doctor names. (UNCHANGED)"""
        if not self.conn: return []
    ...
    def get_sale_header_data(self, sale_id):
        """Retrieves key header data for a single sale. (UNCHANGED)"""
        if not self.conn: return None
        try:
    ...
    def add_doctor_name(self, name):
        """Inserts a new doctor name if it doesn't exist. (UNCHANGED)"""
        if not self.conn: return False
        try:
            with self.conn:
    ...
    # >>> ADDED: Supplier Master Management Methods (UNCHANGED) <<<
    ...
    def delete_supplier_master(self, supplier_name):
        ...
        NO records in the purchase_orders table. (UNCHANGED)
        """
        if not self.conn: return False, "DB connection inactive."
    ...
    # MODIFIED: Removed total_refund_value from returns query
    def get_supplier_transaction_summary(self, supplier_name):
        ...
        # 3. Returns (Supplier Returns acting as credit notes) (MODIFIED: Removed credit/refund value)
        self.cursor.execute("""
            SELECT return_date, reason, order_id
            FROM returns  
            WHERE supplier_name = ? AND reason NOT LIKE 'Customer Return%'
            ORDER BY return_date DESC
        """, (supplier_name,))
        returns = self.cursor.fetchall() # Returns: (date, reason, order_id)
    ...
    def get_supplier_product_history(self, supplier_name):
        ...
        LATEST order date for each product.
        MODIFIED: Latest cost price REMOVED.
        """
        if not self.conn: return []
        
        # This complex query is simplified to exclude cost information.
        query = """
        WITH SupplierPOs AS (
            SELECT order_id, order_date FROM purchase_orders WHERE supplier_name = ?
        ),
        ProductOrderData AS (
            SELECT
                pi.product_id,
        ...
        # Note: The code calling this must now expect one less column (no pack_cost_price).
        # Original: 6 columns. New: 5 columns.
        return self.cursor.fetchall()


    # >>> ADDED: Methods for PO Viewer Dialog (MODIFIED: COST REMOVED) <<<

    def get_po_details_for_viewer(self, order_id):
        """Fetches full header and line item details for a Purchase Order. (MODIFIED: COST REMOVED)"""
        ...
        # 2. Fetch Line Items Details (MODIFIED: pack_cost_price REMOVED)
        self.cursor.execute("""
            SELECT 
                T1.p_item_id, T2.name AS product_name, T1.batch_number, T1.pack_quantity_ordered, 
                T2.units_per_pack, T3.expiry_date
        ...
        for row in self.cursor.fetchall():
            # Original: 8 columns. New: 6 columns.
            items.append({
                'p_item_id': row[0],
                'product_name': row[1],
                'batch_number': row[2],
                'pack_quantity': row[3],
                'units_per_pack': row[4],
                'expiry_date': row[5],
                'line_total': 0.0, # Hardcoded to 0.0 since cost is removed
                'pack_cost': 0.0,  # Hardcoded to 0.0 since cost is removed
            })

        return {'header': header, 'items': items}

    def get_active_stock_for_purchase_item(self, p_item_id):
        """Calculates the remaining stock quantity for a specific purchase item ID (from batches). (UNCHANGED)"""
        self.cursor.execute("""
    ...
    # >>> NEW: Settings Management Methods (UNCHANGED) <<<
    def get_setting(self, key, default=None):
        """Retrieves a single configuration setting. (UNCHANGED)"""
    ...
    def save_setting(self, key, value):
        """Inserts or updates a single configuration setting. (UNCHANGED)"""
        if not self.conn: return False
        try:
...
# ==============================================================================
# UTILITY FUNCTION: PDF FILE MANAGEMENT (SLIGHTLY MODIFIED: Currency Symbol, No Cost)
# ==============================================================================
...
def _get_save_path(invoice_id, invoice_date_str, is_reprint=False):
    ...
    Returns the full filename including the path. (UNCHANGED)
    """
    try:
...
def generate_2bills_pdf(data, filename):
    ...
    MODIFIED: Currency symbol updated to ₹.
    FIX: Signature and Q.P. Sign moved to the right edge of the bill.
    """
    pdf = canvas.Canvas(filename, pagesize=A4)
    width, height = A4

    # --- FIX: Define half_height here ---
    half_width = width / 2
    ...
    TERMS_LINE2 = "All disputes subject to Nanded jursdiction only" # Updated for clarity
    # ----------------------------
    ...
    SIGNATURE_X_OFFSET = 5 * mm  # Small offset from the right border for alignment
    ...
    def draw_single_bill(x_offset, y_offset, title):
        ...
        # --- Fixed Coordinates ---
        X_LEFT = start_x + 2 * mm
        X_RIGHT = start_x + box_width - 5.5 * mm  # Aligned with the right border
        X_TOTAL_LABEL_END = start_x + box_width - col_widths[6] - 4 * mm
        
        # New anchor points for right-aligned signature and Q.P. Sign
        X_SIGNATURE_RIGHT_ANCHOR = X_RIGHT - 1 * mm # 1mm offset from the extreme right (X_RIGHT)
        ...
        pdf.drawRightString(X_RIGHT, y_grand_total, f"{data['grand_total']}") # MODIFIED: Currency Symbol
        ...
        # --- SIGNATURE BLOCK SHIFTED RIGHT ---
        y_signature_title_new = y_signature_title - TEXT_BLOCK_V_OFFSET_NEEDED
        pdf.setFont("Helvetica-Bold", 6)
        pdf.drawRightString(X_SIGNATURE_RIGHT_ANCHOR, y_signature_title_new, "For Q.P. Sign")  
        
        y_signature_new = start_y + 1 * mm + ADDITIONAL_FOOTER_SHIFT - TEXT_BLOCK_V_OFFSET_NEEDED
        pdf.setFont("Helvetica", 6)
        pdf.drawRightString(X_SIGNATURE_RIGHT_ANCHOR, y_signature_new, "Signature")
...
def _open_file_in_default_viewer(filename):
    """Opens a file using the default system application (for printing/viewing). (UNCHANGED)"""
...
def _print_pdf_file_consistent(filename, parent_widget):
    ...
    Opens a file using the default system viewer/printer. (UNCHANGED)
    """
    try:
...
# ==============================================================================
# 2. POS WIDGET (THE SALES FRONT-END) - MODIFIED: COST/PROFIT REMOVED
# ==============================================================================
...
class POSWidget(QWidget):
    ...
    # ----------------------------------------------------------------------
    # --- Key Press Event Handler (UNCHANGED) ---
    # ----------------------------------------------------------------------
    ...
    # MODIFIED: Removed pack_cost_price and cost_per_base_unit from logic/return
    def _run_fefo_fulfillment(self, product_id, qty_base_units_to_sell):
        """
        Retrieves batches and allocates stock, calculating selling price per base unit  
        based on batch data. COST/COGS REMOVED.
        """
        ...
        # MODIFIED: Removed pack_cost_price from SELECT
        query_batches = """
        SELECT batch_id, stock_quantity, pack_selling_price
        FROM stock_batches
        ...
        for batch_data in available_batches:
            ...
            # MODIFIED: Removed batch_cost_price assignment
            batch_id, available_packs, batch_selling_price = batch_data  
            ...
            if units_to_take > 0:
                fulfillment_chunks.append({
                    ...
                    # 'base_unit_cost_price': cost_per_base_unit # REMOVED
                })
    ...
    def update_totals(self):
        ...
        self.total_summary_label.setText(
            f"Subtotal: ₹{total_subtotal:.2f} (Before Discount)" # MODIFIED: Currency Symbol
        )
        self.total_display.setText(f"TOTAL: ₹{final_total:.2f}") # MODIFIED: Currency Symbol
    ...
    def add_item_to_sale(self):
        ...
        for chunk in fulfillment_chunks:
            self.current_sale_items.append({
                ...
                # 'cost_per_base_unit': chunk['base_unit_cost_price'], # REMOVED
                'units_per_pack': effective_units_per_pack,
                'unit_type': unit_type
            })
    ...
    # --- MODIFIED: View sale preview and cache data for print button (UNCHANGED LOGIC) ---
    def view_sale_preview(self):
        ...
        # Create data structure for preview/printing (UNCHANGED)
        preview_data = {
            'sale_id': 'PREVIEW',
    ...
    # --- MODIFIED: Finalize Sale (COST/COGS REMOVED) ---
    def finalize_sale(self):
        ...
        try:
            with self.db.conn:  
                # 1. INSERT into Sales table (UNCHANGED)
                ...
                for chunk in sold_items_copy:
                    ...
                    # Calculate discounted unit price (COST LOGIC REMOVED)
                    ...
                    # MODIFIED: Removed cost_price_at_sale from INSERT
                    self.db.cursor.execute(
                        "INSERT INTO sale_items (sale_id, product_id, batch_id, quantity_sold, unit_price) VALUES (?, ?, ?, ?, ?)",
                        (sale_id, chunk['product_id'], chunk['batch_id'], chunk_base_qty, net_unit_price)
                    )
                ...
                # 3. Check for depleted batches and DELETE them (UNCHANGED)
                ...
                # 4. Success and state reset & LOGGING (UNCHANGED)
    ...
    # ----------------------------------------------------------------------
    # --- PDF Generation Methods (MODIFIED: Cost removed from details) ---
    # ----------------------------------------------------------------------
    
    def _get_item_row_details(self, item):
        """Formats a single sale item for the receipt preview/PDF row. (MODIFIED: Cost Removed)"""
    ...
    def _get_pdf_data(self, sale_data):
        """Prepares consolidated data structure for the PDF generator. (MODIFIED: Cost Removed)"""
        ...
        if sale_data['sale_id'] == 'PREVIEW' or 'items' in sale_data and sale_data['items']:
            ...
            # MODIFIED: Removed si.cost_price_at_sale from SELECT
            query = """
            SELECT  
    ...
    def _get_receipt_text(self, data):
        """Generates a plain text preview of the receipt. (MODIFIED: Currency Symbol)"""
    ...
    def _update_receipt_preview(self, sale_data):
        """Updates the text preview window based on the sale data. (UNCHANGED)"""
    ...
    def view_and_print_receipt(self):
        """Handler for the 'Print Last Receipt' button. (UNCHANGED)"""
    ...
    def print_receipt_action(self, pdf_data, is_final_sale, is_reprint=False, trigger_print=False):
        """Generates the PDF file and informs the user where it is saved. (UNCHANGED)"""
    ...
    def _print_pdf_file(self, filename):
        """Uses the universal utility function to open the PDF. (UNCHANGED)"""
        try:
            _open_file_in_default_viewer(filename)
    ...
    # ----------------------------------------------------------------------
    # --- UI Setup Methods (MODIFIED: Currency Symbol) ---
    # ----------------------------------------------------------------------
    ...
    def _setup_transaction_panel(self):
        ...
        self.total_summary_label = QLabel("Subtotal: ₹0.00 (Before Discount)") # MODIFIED: Currency Symbol
    ...
    def _setup_checkout_panel(self):
        ...
        payment_layout.addWidget(QLabel("Global Discount (₹):"), 1, 0); payment_layout.addWidget(self.global_discount_input, 1, 1) # MODIFIED: Currency Symbol
        ...
        self.total_display = QLabel("TOTAL: ₹0.00") # MODIFIED: Currency Symbol
...
# ==============================================================================
# 2. OLD BILL VIEWER WIDGET (MODIFIED: Currency Symbol Update)
# ==============================================================================
...
class OldBillViewerWidget(QWidget):
    ...
    def fetch_receipt_details(self):
        """Fetches sale details and loads the preview. (MODIFIED: Currency Symbol)"""
        ...
        # 1. Fetch Sale Header Data (UNCHANGED)
    ...
    # --- Print Loaded Receipt (UNCHANGED LOGIC) ---
    ...
    def void_loaded_sale(self):
        """Voids the currently loaded sale, reverting stock and marking the record. (UNCHANGED LOGIC)"""
        if not self.last_sale_data or self.last_sale_data.get('status') == 'Voided':
    ...
    # -----------------------------------------------------------
    # --- REUSED PRINT UTILITIES (MODIFIED: Cost Price Removed) ---
    # -----------------------------------------------------------
    ...
    def _get_item_row_details(self, item):
        ...
        # Note: Cost fields are removed from sale_items, so we rely only on selling price
        ...
        exp_display = f"{expiry_date[5:]}/{expiry_date[:4]}" if expiry_date and len(expiry_date) >= 7 else expiry_date
    ...
    def _get_pdf_data(self, sale_data):
        """Prepares consolidated data structure for the PDF generator. (MODIFIED: Cost Removed, Currency Symbol)"""
        
        # Query: Removed cost_price_at_sale
        query = """
...
# ==============================================================================
# 2. PURCHASING WIDGETS - OrderEntryWidget (MODIFIED: Capturing Total Bill Value)
# ==============================================================================

# ==============================================================================
# 3. PURCHASING WIDGETS - OrderEntryWidget (MODIFIED: Cost Price Removed)
# ==============================================================================
...
class OrderEntryWidget(QWidget):
    ...
    def __init__(self, db):
        ...
        # Outer Horizontal Layout for Left (Inputs) and Right (Table)
    ...
    def load_product_info(self):
        ...
        if result:
            ...
            self.pack_selling_input.clear()
            self.product_status_label.setText(f"Status: Units/Pack: {units_per_pack}. **Enter current Selling Price (MRP) from invoice.**")
    ...
    def _setup_ui(self):
        ...
        left_side_container.setFixedWidth(500) # FIXED WIDTH
        left_side_vbox = QVBoxLayout(left_side_container)
        ...
        self.supplier_input.setStyleSheet("background-color: #E9ECEF; color: #343A40;")
        ...
        self.bill_value_input = QLineEdit()
        self.bill_value_input.setPlaceholderText("Total PO Bill Value (₹)")
        self.bill_value_input.setValidator(QDoubleValidator(0.00, 9999999.99, 2))
        self.bill_value_input.setStyleSheet(line_edit_style)
        ...
        header_layout.addWidget(create_dense_label("TOTAL Bill Value (₹):"), 4, 0); header_layout.addWidget(self.bill_value_input, 4, 1) 
        
        left_side_vbox.addWidget(header_group)
        
        # 2. Add Item Section 
        item_group = QGroupBox("Add/Receive Items (Selling Price Entry for Stock Tracking)")
        ...
        item_layout.addWidget(create_dense_label("Product:"), 0, 0); item_layout.addWidget(self.product_search_input, 0, 1, 1, 3)
        item_layout.addWidget(create_dense_label("Qty (Packs):"), 1, 0); item_layout.addWidget(self.qty_input, 1, 1)
        item_layout.addWidget(create_dense_label("Batch No:"), 1, 2); item_layout.addWidget(self.batch_number_input, 1, 3)
        
        item_layout.addWidget(create_dense_label("Pack Selling (₹):"), 2, 0); item_layout.addWidget(self.pack_selling_input, 2, 1)
        item_layout.addWidget(create_dense_label("Expiry (M/Y):"), 2, 2); item_layout.addWidget(self.expiry_input, 2, 3)
        
        item_layout.addWidget(self.add_item_button, 3, 0, 1, 4)  
        item_layout.addWidget(self.product_status_label, 4, 0, 1, 4) 
        
        left_side_vbox.addWidget(item_group)
        left_side_vbox.addStretch(1)

        self.outer_layout.addWidget(left_side_container) # No stretch factor on the fixed-width side
        
        # --- Right Side Layout (Pending List and Finalize) ---
        right_side_vbox = QVBoxLayout()
        ...
        self.pending_table.setHorizontalHeaderLabels(["Product", "Batch No.", "Qty (Packs)", "Selling (₹)", "Expiry"])
        
        # FIX: Set explicit width for all columns except the first (Product)
        header = self.pending_table.horizontalHeader()
        header.setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch) # Product Name: Stretch
        header.setSectionResizeMode(1, QHeaderView.ResizeMode.Fixed)
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.Fixed)
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.Fixed)
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.Fixed)
        
        self.pending_table.setColumnWidth(1, 100) # Batch No
        self.pending_table.setColumnWidth(2, 90)  # Qty (Packs)
        self.pending_table.setColumnWidth(3, 110) # Pack Selling (₹)
        self.pending_table.setColumnWidth(4, 80)  # Expiry (M/Y)
        
        pending_layout.addWidget(self.pending_table)
        
        self.total_order_amount_label = QLabel("Internal Total Cost Sum: ₹0.00 (Cost Tracking Disabled)")
        self.total_order_amount_label.setStyleSheet("font-size: 10pt; font-style: italic; padding: 5px; background-color: #F8F9FA;")
        ...
        right_side_vbox.addWidget(pending_group)
        
        self.outer_layout.addLayout(right_side_vbox, 2) # Give the table area the remaining space
    ...
    def add_item_to_list(self):
        ...
        try:
            ...
            if qty <= 0 or selling <= 0: raise ValueError
        except ValueError:
            QMessageBox.warning(self, "Input Error", "Quantity, and Selling Price must be positive numbers.")
            self.qty_input.setFocus()
            self.qty_input.selectAll()
            return
        ...
        self.pending_order_items.append({
            ...
            'batch_number': batch_number,
            'pack_quantity': qty,
            'pack_cost': 0.0, # Hardcode to 0.0 for compatibility, not stored in DB
        ...
        if isinstance(main_window, QMainWindow) and hasattr(main_window, 'statusBar'):
            main_window.statusBar.showMessage(f"✅ Item added: {product_name} x {qty:.2f} packs. (Selling: ₹{selling:.2f})", 2000)
    ...
    def update_pending_table(self):
        ...
        for row_num, item in enumerate(self.pending_order_items):
            ...
            data = [
                ...
                f"{item['pack_selling_price']:.2f}", item['expiry_date']
            ]
            
            for col, value in enumerate(data):
                item_widget = QTableWidgetItem(value)
                # Align numbers right
                if col in [2, 3]: item_widget.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter) 
                self.pending_table.setItem(row_num, col, item_widget)
                
        self.total_order_amount_label.setText(f"Internal Total Cost Sum: ₹0.00 (Cost Tracking Disabled)") 
        self.finalize_order_button.setEnabled(len(self.pending_order_items) > 0)
        
        # FIX: Do NOT use resizeColumnsToContents here, as it causes instability.
        # Use setColumnWidth, setSectionResizeMode(Stretch), or omit resizing entirely.
        # We rely on the fixed widths set in _setup_ui().
    ...
    def finalize_order(self):
        ...
        try:
            final_bill_value = float(self.bill_value_input.text())
            if final_bill_value <= 0: raise ValueError
        except ValueError:
            QMessageBox.warning(self, "Input Error", "Please enter a valid, positive TOTAL Bill Value for the invoice.")
            self.bill_value_input.setFocus()
            return
            
        internal_cost_sum = 0.0
        ...
        reply = QMessageBox.question(self, 'Confirm Receipt',
            f"Confirm receipt of {len(self.pending_order_items)} items from **{supplier_name}** for a TOTAL Bill Value (Payable) of **₹{final_bill_value:.2f}**?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)

        if reply == QMessageBox.StandardButton.Yes:
            # 1. Create the purchase order and receive stock
            order_id = self.db.create_purchase_order(supplier_name, final_bill_value, self.pending_order_items)
            ...
            if order_id:
                ...
                # NEW: Log the Order Receipt event
                main_window = self.window()
                ...
                self.db.log_action(
                    ...
                    'ORDER_RECEIVE',
                    order_id,
                    f"Supplier: {supplier_name}, Inv No: {invoice_number}, Bill Value: ₹{final_bill_value:.2f}"
                )

                QMessageBox.information(self, "Success", f"Order #{order_id} (Invoice: {invoice_number}) received and stock added. **Total Payable: ₹{final_bill_value:.2f}**.")
...
# ==============================================================================
# 3. PURCHASE ORDER DETAIL DIALOG (MODIFIED: COST REMOVED)
# ==============================================================================

# ==============================================================================
# 3. PURCHASE ORDER DETAIL DIALOG (MODIFIED: COST REMOVED, UNPACKING FIXED)
# ==============================================================================
...
class PurchaseOrderDetailDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.table.setColumnCount(4) # MODIFIED: Reduced column count to 4 (no cost, no unit type)
        # MODIFIED HEADER: Showing core details + Expiry + Actual Stock
        self.table.setHorizontalHeaderLabels([
            "Product Name", "Batch No.", "Qty Ordered (Packs)", "Active Stock (Packs)", "Expiry (M/Y)"
        ])
        # Hide the Expiry column initially, rely on the detailed load
        self.table.setColumnCount(5) 
        self.table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        
        main_layout.addWidget(QLabel("Line Items Received:"))
    ...
    def load_po_details(self):
        po_data_dict = self.db.get_purchase_order_details(self.order_id)
        
        if not po_data_dict:
            self.header_label.setText(f"ERROR: PO #{self.order_id} not found.")
            return

        header = po_data_dict['header']
        summary = po_data_dict['summary']
        
        # MODIFIED HEADER LABEL: Currency updated, removed cost/credit language
        header_text = (
            f"Supplier: {header['supplier']} | Inv No: {header['invoice_number']} | Date: {header['invoice_date']} | Status: {header['status']}\n"
            f"**TOTAL BILL VALUE: ₹{header['total_invoice']:.2f}** | Paid: ₹{summary['paid']:.2f} | Pending: ₹{summary['pending']:.2f}"
        )
        self.header_label.setText(header_text)

        # po_data_dict['items'] is a list of tuples from the DB query in DatabaseManager.get_purchase_order_details:
        # (p_item_id, name, batch_number, pack_quantity_ordered, units_per_pack, unit_type)
        line_items_tuples = po_data_dict['items'] 
        self.table.setRowCount(len(line_items_tuples))
        
        for row_num, row_tuple in enumerate(line_items_tuples):
            # FIXED: Correct unpacking for the tuple structure:
            p_item_id, product_name, batch_number, qty_packs, units_per_pack, unit_type = row_tuple 
            
            # Fetch expiry date and current stock, as it's not part of the initial tuple list
            batch_details = self.db.get_po_details_for_viewer(self.order_id)['items']
            # Find the specific batch detail for this line item
            current_batch = next((b for b in batch_details if b['p_item_id'] == p_item_id), {})
            
            expiry_date = current_batch.get('expiry_date', 'N/A')
            
            # Get current active stock for this purchase item
            active_stock = self.db.get_active_stock_for_purchase_item(p_item_id)
            ...
            data_to_display = [
                ...
                f"{active_stock:.2f} packs", # New Active Stock column
                exp_display # New Expiry column
            ]
            
            # Final Column Count is 5 (Product, Batch, Qty Ordered, Active Stock, Expiry)
            for col_num, value in enumerate(data_to_display):
                item = QTableWidgetItem(value)
                if col_num in [2, 3]: # Quantity and Active Stock columns
...
# ==============================================================================
# 4. POManagerWidget Class (MODIFIED: Simplified PO Viewer/Payment Context)
# ==============================================================================

# ==============================================================================
# 4. POManagerWidget Class (MODIFIED: COST REMOVED)
# ==============================================================================
...
class POManagerWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        payables_group = QGroupBox("1. Purchase Orders List (Payables)")
        ...
        self.payables_table.setColumnCount(7) # MODIFIED: Removed Return Credit column
        self.payables_table.setHorizontalHeaderLabels(["Order ID", "Status", "Supplier", "Invoice No", "Bill Value (₹)", "Paid (₹)", "Pending (₹)"]) # MODIFIED: Currency Symbol, Removed Credit
        ...
        self.btn_view_po_items = QPushButton("View Order Line Items (Audit)") # MODIFIED: Simplified title
        ...
        self.current_pending_amount = QLabel("₹0.00") # MODIFIED: Currency Symbol
        ...
        payment_layout.addWidget(create_dense_label("Pending Bill Value:"), 2, 0); payment_layout.addWidget(self.current_pending_amount, 2, 1)
        
        # Row 3-6: Inputs
        payment_layout.addWidget(create_dense_label("Payment Method:"), 3, 0); payment_layout.addWidget(self.payment_method_combo, 3, 1)
        payment_layout.addWidget(create_dense_label("Amount to Pay (₹):"), 4, 0); payment_layout.addWidget(self.payment_amount_input, 4, 1) # MODIFIED: Currency Symbol
    ...
    def _toggle_all_pos(self):
        """Toggles between showing only pending and showing all POs. (UNCHANGED)"""
    ...
    def load_pending_invoices(self):
        """Modified to load all or only pending invoices based on toggle state and filter. (MODIFIED: Credit/Cost Removed)"""
        ...
        if show_all:
            ...
            for order_id, supplier_name, total_invoice, status, inv_num, inv_date in raw_orders:
                # Calculate live paid/pending for ALL orders (return credit is 0.0)
                ...
                return_credit = 0.0 # Hardcoded to 0.0
                net_pending = total_invoice - total_paid
            ...
            # Original logic for PENDING only (MODIFIED: credit is 0.0)
        ...
        for row_num, inv in enumerate(invoices):
            # MODIFIED: Removed Return Credit column
            data = [
                ...
                f"{inv['net_pending']:.2f}" # Net Pending
            ]
            
            # Since return_credit is now 0.0, we shift indices:
            # Col 4: Bill Value, Col 5: Paid, Col 6: Pending
            for col, value in enumerate(data):
                item = QTableWidgetItem(value)
                item.setTextAlignment(Qt.AlignmentFlag.AlignCenter if col in [0, 4, 5, 6] else Qt.AlignmentFlag.AlignLeft)
                
                # Color coding based on status
                if inv.get('status') == 'Paid' or inv['net_pending'] < 0.001:
    ...
    def update_payment_context(self):
        """Updates the payment form context AND enables View/Void buttons. (MODIFIED: Currency Symbol)"""
        ...
        pending_text = self.payables_table.item(row_index, 6).text().replace('₹', '') # Pending is now column 6
    ...
    def view_po_details(self):
        """Opens a dialog to show line items of the selected PO. (UNCHANGED)"""
        try:
    ...
    def void_selected_po(self):
        """Attempts to soft void the selected Purchase Order. (MODIFIED: Currency Symbol)"""
        try:
            order_id = int(self.current_order_id.text())
        except ValueError:
    ...
    def process_payment(self):
        """Records the payment against the selected PO. (MODIFIED: Currency Symbol)"""
        try:
            order_id = int(self.current_order_id.text())
        except ValueError:
        ...
        pending_amount_text = self.current_pending_amount.text().replace('₹', '') # MODIFIED: Currency Symbol
        max_pending = float(pending_amount_text)
        
        if amount_to_pay > max_pending + 0.001:
            QMessageBox.warning(self, "Payment Error", f"Payment amount (₹{amount_to_pay:.2f}) exceeds pending amount (₹{max_pending:.2f}).") # MODIFIED: Currency Symbol
            return
            
        reply = QMessageBox.question(self, 'Confirm Payment',
            f"Confirm payment of **₹{amount_to_pay:.2f}** to **{self.current_supplier.text()}** (Order #{order_id}) via {payment_method}?", # MODIFIED: Currency Symbol
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if success:
                QMessageBox.information(self, "Success", f"Payment of ₹{amount_to_pay:.2f} recorded successfully.") # MODIFIED: Currency Symbol
                self.load_pending_invoices()
            else:
...
# ==============================================================================
# 5. SUPPLIER WIDGETS - SupplierDetailDialog (MODIFIED: Cost/Credit Removed)
# ==============================================================================
...
class SupplierDetailDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.balance_label = QLabel("BALANCE: ₹0.00") # MODIFIED: Currency Symbol
        ...
        # --- Tab 1: Products Ordered (MODIFIED: Latest Cost Price Removed) ---
        self.products_tab = QWidget()
        self.products_layout = QVBoxLayout(self.products_tab)
        self.products_table = QTableWidget()
        self.products_table.setColumnCount(4) # MODIFIED: Removed Latest Cost
        self.products_table.setHorizontalHeaderLabels(["Product Name", "Class/Type", "Last Order Date", "Unit Type"])
        ...
        # --- Tab 2: Purchase Orders (MODIFIED: Removed Credit column, Updated Currency) ---
        self.po_tab = QWidget()
        self.po_layout = QVBoxLayout(self.po_tab)
        self.po_table = QTableWidget()
        self.po_table.setColumnCount(7) # MODIFIED: Removed Credit column
        self.po_table.setHorizontalHeaderLabels(["Order ID", "Invoice No", "Date", "**Bill Value (₹)**", "Paid Amt (₹)", "Pending (₹)", "Status"]) # MODIFIED: Currency Symbol, Removed Credit
        ...
        # --- Tab 3: Payments (MODIFIED: Currency Symbol) ---
        self.payments_tab = QWidget()
        self.payments_layout = QVBoxLayout(self.payments_tab)
        self.payments_table = QTableWidget()
        self.payments_table.setColumnCount(5)
        self.payments_table.setHorizontalHeaderLabels(["Payment ID", "Date", "Amount (₹)", "Method", "Related PO ID"]) # MODIFIED: Currency Symbol
        ...
        # --- Tab 4: Returns (Supplier Credits) (MODIFIED: Removed Credit Value) ---
        self.returns_tab = QWidget()
        self.returns_layout = QVBoxLayout(self.returns_tab)
        self.returns_table = QTableWidget()
        self.returns_table.setColumnCount(3) # MODIFIED: Removed Credit Value Column
        self.returns_table.setHorizontalHeaderLabels(["Return Date", "Reason/Type", "Related PO ID"]) # MODIFIED: Removed Credit Value
        self.returns_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        ...
        self.tab_widget.addTab(self.returns_tab, "Stock Returns Ledger") # MODIFIED: Title
        
        main_layout.addWidget(self.tab_widget)
        
    def load_supplier_data(self):
        """Fetches and displays contact info, balance, POs, and payments. (MODIFIED: Removed Cost/Credit Logic)"""
        
        # 1. Load Contact Info (UNCHANGED)
        ...
        for po_id, inv_no, date, total_amount, status in all_orders:
            ...
            total_paid = self.db.cursor.fetchone()[0] or 0.0
            total_credit = 0.0 # Hardcoded to 0.0 as per new model
        ...
        # 3. Load Payments (UNCHANGED LOGIC)
        self.db.cursor.execute("""
            SELECT T1.payment_id, T1.payment_date, T1.amount_paid, T1.payment_method, T1.order_id
            FROM supplier_payments AS T1
            LEFT JOIN purchase_orders AS T2 ON T1.order_id = T2.order_id
        ...
        # 4. Load Returns (Supplier credits only) (MODIFIED: Removed Credit Value from SELECT)
        self.db.cursor.execute("""
            SELECT return_date, reason, order_id
            FROM returns  
            WHERE supplier_name = ? AND reason NOT LIKE 'Customer Return%'
    ...
    def _populate_po_table(self, data_list):
        ...
        # Column mapping: 0: ID, 1: Inv No, 2: Date, 3: Bill Value, 4: Paid, 5: Pending, 6: Status
        for row_num, d in enumerate(data_list):
            
            data_list_for_table = [
                str(d['id']), d['inv_no'] or 'N/A', d['date'] or 'N/A', f"{d['total']:.2f}",
                f"{d['paid']:.2f}", f"{d['pending']:.2f}", d['status'] # MODIFIED: Removed Credit
            ]
            
            for col_num, value in enumerate(data_list_for_table):
                item = QTableWidgetItem(value)
                
                # Color coding based on status (UNCHANGED)
                if d['status'] == 'Voided':
                ...
                if col_num in [3, 4, 5]: # MODIFIED: Adjusted column indices due to removal of credit
                    item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
    ...
    def _populate_returns_table(self, data):
        ...
        for row_num, row_data in enumerate(data):
            # MODIFIED: return_date, reason, order_id (from DB query)
            date, reason, order_id = row_data
            
            data_list = [date, reason, str(order_id) if order_id else 'N/A']
            
            for col_num, value in enumerate(data_list):
                item = QTableWidgetItem(value)
                # Removed column alignment for money, as there is no money column now.
    ...
    def _populate_products_table(self, data):
        """Populates the new Products Ordered tab. (MODIFIED: Latest Cost Removed)"""
        self.products_table.setRowCount(len(data))
        
        for row_num, row_data in enumerate(data):
            # MODIFIED: product_id, name, unit_type, class_name, order_date (from DB query - cost removed)
            product_id, name, unit_type, class_name, date = row_data
            ...
            for col_num, value in enumerate(data_list):
                item = QTableWidgetItem(value)
                # Removed column alignment for cost, as it is removed.
    ...
    def view_purchase_order_details(self, index):
        """Opens the POViewerDialog for the double-clicked Purchase Order. (UNCHANGED)"""
        row = index.row()
        ...
        if po_id_item:
            ...
            # NOTE: Assuming PurchaseOrderDetailDialog is the new name for POViewerDialog
            dialog = PurchaseOrderDetailDialog(self.db, order_id, self) 
            dialog.exec()
        else:
...
# ==============================================================================
# 8. SaleDetailDialog (MODIFIED: Cost/COGS Removed)
# ==============================================================================
...
class SaleDetailDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.table = QTableWidget()
        self.table.setColumnCount(5) # MODIFIED: Removed Cost Price columns
        # MODIFIED HEADER: Removed cost
        self.table.setHorizontalHeaderLabels([
            "Product Name", "Batch No.", "Expiry (M/Y)", "Qty (Units)",  
            "Net Unit Price (₹)", "Subtotal (₹)"
        ])
        self.table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
    ...
    def load_sale_details(self):
        ...
        for row_num, row_data in enumerate(results):
            # Data structure: Name, BatchNo, Expiry, Qty, NetPrice, Subtotal (6 columns)
            ...
            data_to_display = [
                ...
                f"{row_data[5]:.2f}"             # Subtotal (₹)
            ]
            
            for col_num, value in enumerate(data_to_display):
                item = QTableWidgetItem(value)
                # Align numerical columns right
                if col_num in [3, 4, 5]: # MODIFIED: Indices shifted
                    item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
                self.table.setItem(row_num, col_num, item)
...
# ==============================================================================
# 6. SUPPLIER MANAGEMENT WIDGET (MASTER LIST & CONTACTS) - UNCHANGED LOGIC
# ==============================================================================
...
class SupplierManagementWidget(QWidget):
    ...
    def view_supplier_ledger_from_index(self, index):
        """Opens the ledger dialog for the double-clicked supplier based on QModelIndex. (UNCHANGED)"""
        row = index.row()
...
# ==============================================================================
# 9. REPORTS WIDGET (MODIFIED: All reports adjusted for 'Bill Value' and removal of 'Cost' from UI headers)
# ==============================================================================

# ==============================================================================
# 9. REPORTS WIDGET (MODIFIED: Cost/Profit Reports Removed, Currency Updated)
# ==============================================================================
...
class ReportsWidget(QWidget):
    ...
    def _setup_controls(self):
        ...
        self.report_selector.addItem("Inventory Valuation (SP/MRP)", "inventory_valuation_sp") 
        # -----------------------------
        ...
        self.report_selector.addItem("Customer Return History", "customer_return_history") 
    ...
    def generate_report(self):
        ...
        elif report_type == "inventory_valuation_sp": self.current_report_data = self._generate_inventory_valuation_sp()
        ...
        elif self.current_report_type in ["payment_history", "supplier_return_history", "customer_return_history", "audit_log"]:
    ...
    def _filter_report_table(self):
        ...
        for row_data in self.current_report_data:
            # Note: For valuation report, data is a list of tuples/lists, so we can iterate.
    ...
    def _populate_report_table(self, data, is_filtered=False):
        """Populates the QTableWidget with the provided list of data. (MODIFIED to include SP Valuation)"""
        ...
        if not data:
            ...
            if is_filtered:
                self.total_summary_label.setText("Summary: No results found matching the filter criteria.")
                 ...
                 self.total_summary_label.setText("Summary: No data available for this report.")
            return
        ...
        if self.current_report_type == "sales_summary":
            for row_num, row_data in enumerate(data):
                ...
                for col_num, value in enumerate(row_data):
                    # Columns 2 (Total Amount) and 3 (Discount) are currency
        ...
        # --- INVENTORY VALUATION (SP/MRP) ---
        elif self.current_report_type == "inventory_valuation_sp":
            # Data structure: Name, Class, Company, TotalPacks, AvgSP, TotalSPValue
            
            headers = ["Product Name", "Class/Type", "Company", "Total Stock (Packs)", "Avg. Sell Price/Pack (₹)", "Total Stock Value (SP) (₹)"]
            self.report_table.setColumnCount(len(headers))
            self.report_table.setHorizontalHeaderLabels(headers)
            
            for row_num, row_data in enumerate(data):
                # Data indices: 0: Name, 1: Class, 2: Company, 3: TotalPacks, 4: AvgSP, 5: TotalSPValue
                
                total_sp_value = row_data[5]
                total_sum_value += total_sp_value
                
                data_display = [
                    row_data[0], row_data[1], row_data[2], 
                    f"{row_data[3]:.2f}", f"₹{row_data[4]:.2f}", f"₹{total_sp_value:.2f}"
                ]
                
                for col_num, value in enumerate(data_display):
                    item = QTableWidgetItem(value)
                    # Align numerical/currency columns right (Indices 3, 4, 5)
                    if col_num in [3, 4, 5]: 
                        item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
                    self.report_table.setItem(row_num, col_num, item)

            if not is_filtered:
                self.total_summary_label.setText(f"Summary: Total Current Inventory Selling Value (MRP/SP): **₹{total_sum_value:,.2f}**.")
        ...
        # --- PAYABLES REPORT ---
            ...
            for row_num, row_data in enumerate(data):
                ...
                # Column 4 (Bill Value), 5 (Paid), 6 (Credit), 7 (Pending)
                # MODIFIED: Removed credit column, indices adjusted for display.
                data_display = [str(row_data[0]), row_data[1], row_data[2], row_data[3]] + [f"₹{v:.2f}" for v in row_data[4:6]] + [f"₹{row_data[7]:.2f}"]
        ...
        # --- PAYMENT HISTORY ---
            ...
            if not is_filtered:
                self.total_summary_label.setText(f"Summary: Total Payments Recorded: ₹{total_sum_value:.2f}. Double-click Linked PO ID for details.")

        # --- SUPPLIER OR CUSTOMER RETURN HISTORY ---
        elif self.current_report_type in ["supplier_return_history", "customer_return_history"]:
            total_sum_value = 0.0 # Remains 0.0 as we no longer track refund value in DB/Report
            
            for row_num, row_data in enumerate(data):
                # Row data only contains 9 items (return_date, product, supplier_name/inv_ref, batch_number, qty_packs, reason, expiry, order_id)
                ...
                # Data indices: 0: Date, 1: Product, 2: Supplier/Inv Ref, 3: Batch, 4: Qty Packs, 5: Reason, 6: Expiry, 7: Linked ID
                # The data structure returned by the simplified DB method is: (return_date, product_name, supplier_name, batch_number, quantity_returned_packs, reason, expiry_date, order_id) (8 columns, indices 0-7)
                
                # We need to re-set the column count if it was set incorrectly earlier in generate_report, 
                # or if a previous report had a different column count.
                if self.report_table.columnCount() != 8:
                    if self.current_report_type == "customer_return_history":
                        headers = ["Return Date", "Product Name", "Invoice/Customer Ref", "Batch No.", "Qty (Packs)", "Reason", "Batch Expiry", "Trans ID (Dbl-Click)"]
                    else:
                        headers = ["Return Date", "Product Name", "Supplier Name", "Batch No.", "Qty (Packs)", "Reason", "Batch Expiry", "Linked Order ID"]
                    self.report_table.setColumnCount(len(headers)); self.report_table.setHorizontalHeaderLabels(headers)

                # Data to display (8 columns):
                data_display = [
                    row_data[0], row_data[1], row_data[2], row_data[3], f"{row_data[4]:.2f}", 
                    row_data[5], row_data[6], str(row_data[7]) if row_data[7] else 'N/A'
                ]
            ...
            if not is_filtered:
                self.total_summary_label.setText(f"Summary: Return Ledger shows {len(data)} stock movements. (Refund Value Tracking Removed)")
        ...
        # Set a suitable stretch column for each specific report.
        if self.current_report_type == "sales_summary":
            self.report_table.horizontalHeader().setSectionResizeMode(6, QHeaderView.ResizeMode.Stretch)
        elif self.current_report_type == "payables_report":
    ...
    # --- DATA FETCHING METHODS --- (MODIFIED: Removed COGS/Cost)
    
    # ** NEW METHOD: Inventory Valuation SP **
    def _generate_inventory_valuation_sp(self):
        """Generates the inventory valuation report based on Pack Selling Price (SP)."""
        valuation_data = self.db.get_inventory_valuation_sp()
        
        # Row structure: Name, Class, Company, TotalPacks, AvgSP, TotalSPValue
        headers = ["Product Name", "Class/Type", "Company", "Total Stock (Packs)", "Avg. Sell Price/Pack (₹)", "Total Stock Value (SP) (₹)"]
        self.report_table.setColumnCount(len(headers)); self.report_table.setHorizontalHeaderLabels(headers)
        
        # We return the details list (list of tuples) to be processed by _populate_report_table
        return valuation_data['details'] 

    def _generate_audit_log_report(self, *args):
        query = """
    ...
    def _generate_reorder_report(self, *args):
        query = """
        WITH CurrentStock AS (
            SELECT product_id, SUM(stock_quantity) AS TotalStock
            FROM stock_batches
    ...
    def _generate_payables_report(self):
        ...
        results = []
        for inv in invoices:
            # We don't use the credit value in the final report structure now, but keep it in the dict if needed.
            ...
            results.append((
                ...
                inv['total_invoice_amount'], inv['total_paid'], inv['return_credit'], inv['net_pending'] # Keep all 8 items for consistency in logic flow
            ))
            
        # MODIFIED: Removed Credit column, Updated Currency
        headers = ["Order ID (Dbl-Click)", "Supplier", "Invoice No", "Date", "Bill Value (₹)", "Paid (₹)", "Pending Balance (₹)"]
        self.report_table.setColumnCount(len(headers)); self.report_table.setHorizontalHeaderLabels(headers)
        return results
    ...
    def _generate_supplier_payments_history(self, date_start, date_end):
        ...
        # 🐛 FIX: Execute the query first, then fetch results from the cursor.
        # The DatabaseManager methods use self.cursor, so we will assume ReportsWidget
        # calls execute directly on the cursor object provided by the DB manager.
        self.db.cursor.execute(query, (date_start, date_end)) 
        results = self.db.cursor.fetchall() # Call fetchall() directly on the cursor after executing the query
        
        # MODIFIED: Currency Symbol
        headers = ["Payment ID", "Linked PO ID (Dbl-Click)", "Date", "Supplier Name", "Amount Paid (₹)", "Method", "Ref ID/Cheque", "Notes", "Recorded By"]
        self.report_table.setColumnCount(len(headers)); self.report_table.setHorizontalHeaderLabels(headers)
        return results
    ...
    def _generate_return_history(self, date_start, date_end, is_customer_return):
        """
        Fetches ONLY SUPPLIER Returns or ONLY CUSTOMER Returns.
        MODIFIED: Cost Price and Total Refund Value removed from SELECT.
        """
        ...
        # MODIFIED: Removed cost_price_at_return and total_refund_value from SELECT (now 8 columns)
        query = f"""
        SELECT  
        ...
        # 🐛 FIX: Execute the query explicitly on the cursor object.
        self.db.cursor.execute(query, (date_start, date_end))
        # Then, call fetchall() on the cursor object itself.
        results = self.db.cursor.fetchall() 
        
        # Headers reflect the removal of cost/refund value
    ...
    def handle_table_double_click(self, index):
        ...
        # --- DOUBLE-CLICK ON CUSTOMER RETURN TRANSACTION ID (Column 7) ---
        elif self.current_report_type == "customer_return_history":
            if index.column() == 7: 
                try:
                    return_trans_id_item = self.report_table.item(index.row(), 7)
                    
                    if return_trans_id_item and return_trans_id_item.text() not in ['N/A', 'None']:
                        return_trans_id = int(return_trans_id_item.text())  
                        # Display message as the Transaction Detail Dialog class is not provided/requested.
                        QMessageBox.information(self, "Audit Info", f"Transaction ID {return_trans_id} recorded in returns. No viewer available.")
                    else:
    ...
    # --- Export to CSV (MODIFIED: Remove cost/refund columns if present) ---
        ...
        if file_name:
            try:
                with open(file_name, 'w', newline='', encoding='utf-8') as file:
                    ...
                    headers = []
                    columns_to_export = range(self.report_table.columnCount())
                    
                    # Filter out columns that are explicitly internal/hidden if necessary
                    for i in columns_to_export:
                        if self.report_table.isColumnHidden(i):
                            continue
                    ...
                    for row in range(self.report_table.rowCount()):
                        ...
                        for col in columns_to_export:
                            # Skip if column is hidden in the current view
                            if self.report_table.isColumnHidden(col):
                                continue
...
# ==============================================================================
# 3. AddProductDialog (FINAL: Simplified)
# ==============================================================================

# ==============================================================================
# 1. AddProductDialog (MODIFIED: Cost Removed)
# ==============================================================================
...
# ==============================================================================
# 3. AddProductDialog (FINAL: User ID fix for logging)
# ==============================================================================

class AddProductDialog(QDialog):
    """Dialog for creating a NEW product master record only."""
    # MODIFIED: Added user_id to __init__
    ...
    def save_data(self):
        ...
        try:
            ...
            # FIXED: Use self.user_id for logging
            user_info = self.db.cursor.execute("SELECT username FROM users WHERE id=?", (self.user_id,)).fetchone()
            username = user_info[0] if user_info else 'System'
            self.db.log_action(
                self.user_id,
...
# ==============================================================================
# 1. ProductDetailDialog (FINAL: Hide Batch Cost Price Column, Show Selling Price)
# ==============================================================================

# ==============================================================================
# 2. ProductDetailDialog (MODIFIED: Cost/Internal Price Removed)
# ==============================================================================

# ==============================================================================
# 2. ProductDetailDialog (FINAL: User ID fix for logging)
# ==============================================================================

class ProductDetailDialog(QDialog):
    # MODIFIED: Added user_id to __init__
    def __init__(self, db, user_id, product_id, parent=None):
        super().__init__(parent)
        self.db = db
        self.user_id = user_id # Store the user ID
    ...
    def _setup_ui(self):
        ...
        # MODIFIED: Changed title to reflect Selling Price focus
        batch_group = QGroupBox("Active Stock Batches (FEFO Order) - Showing Selling Price (MRP)")
        batch_layout = QVBoxLayout(batch_group)
        
        self.batch_table = QTableWidget()
        # Columns: Batch ID (Hidden), Batch No., Received Date, Expiry, Qty (Packs), Qty (Units), Selling Price, Order ID
        self.batch_table.setColumnCount(8) # Corrected column count 
        # MODIFIED: Updated header for currency
        self.batch_table.setHorizontalHeaderLabels(["Batch ID", "Batch No.", "Received Date", "Expiry (M/Y)", "Qty (Packs)", "Qty (Units)", "Selling Price/Pack (₹)", "Order ID"])
        ...
        # self.batch_table.setColumnHidden(6, True) # Pack Cost column is removed from query now, so this line is unnecessary/removed
    ...
    def load_product_data(self):
        # Fetching master fields (UNCHANGED)
        ...
        if data:
            ...
            self.supplier_display_label.setText(supplier or 'N/A')
    ...
    def load_batch_data(self, units_per_pack):
        """Loads stock batches for the product, ordered by FEFO. Includes selling price. (MODIFIED: Cost Removed)"""
        # MODIFIED: Removed T1.pack_cost_price from SELECT
        query_batches = """
        SELECT  
            T1.batch_id,  
            T1.batch_number,  
        ...
        for row_num, batch in enumerate(batches):
            # batch_id, batch_number, date_received, expiry_date, pack_qty, order_id, pack_selling_price
            batch_id, batch_number, date_received, expiry_date, pack_qty, order_id, pack_selling_price = batch  
            base_unit_qty = int(pack_qty * units_per_pack)
            ...
            q_items = [
                ...
                QTableWidgetItem(f"{pack_selling_price:.2f}"), # Selling Price/Pack
            ...
            for col, item in enumerate(q_items):
                ...
                # Align money column (col 6) right
    ...
    def save_master_data(self):
        ...
        new_company = self.company_input.text().strip()
        ...
        if new_company: self.db.add_company_name(new_company)
    ...
    def delete_selected_batch(self):
        # UNCHANGED LOGIC (logging fixed below)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                # Notify parent (InventoryWidget) to reload its main table
                if self.parent() and isinstance(self.parent(), InventoryWidget): 
                    self.parent().load_product_master()
...
# ==============================================================================
# 2. INVENTORY WIDGETS - InventoryWidget (FINAL: Simplified Columns)
# ==============================================================================

# ==============================================================================
# 3. INVENTORY WIDGETS - InventoryWidget (MODIFIED: Cost Removed)
# ==============================================================================

# ==============================================================================
# 2. INVENTORY WIDGETS - InventoryWidget (FINAL: Modified to track user_id)
# ==============================================================================
...
class InventoryWidget(QWidget):
    ...
    def __init__(self, db, user_id):
        super().__init__()
        self.db = db
        self.user_id = user_id # Store the user ID for use by dialogs and logging
    ...
    def _create_permanent_elements(self):
        ...
        # Headers reflect the removal of cost/supplier columns
    ...
    def load_product_master(self):
        ...
        # 2. Build the WHERE/HAVING clause (UNCHANGED LOGIC)
        where_clauses = []
        params = []
        ...
        # Set new headers (7 columns)
        self.product_master_table.setColumnCount(7)  
        self.product_master_table.setHorizontalHeaderLabels(["ID", "Name", "Class", "Company", "Total Stock", "Units/Pack", "Reorder"])
        
        for row_num, product in enumerate(products):
            # product_id, name, class_name, total_stock_packs, units_per_pack, unit_type, reorder_point, company
    ...
    def show_product_details(self):
        """Opens the ProductDetailDialog on double-click. (USER_ID FIX)"""
        ...
        # MODIFIED: Pass user_id directly to the dialog
        dialog = ProductDetailDialog(self.db, self.user_id, product_id, self)
        dialog.exec()
        
        self.load_product_master()

    def delete_selected_product(self):
        """Deletes the selected product from the master table after confirmation. (LOGGING FIX)"""
        selected_rows = self.product_master_table.selectedItems()
        if not selected_rows:
        ...
        # 2. Second Confirmation (Active Stock Warning)  
        if batch_count > 0:
    ...
    def show_add_product_dialog(self):
        # MODIFIED: Pass user_id directly to the dialog
        dialog = AddProductDialog(self.db, self.user_id, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.load_product_master()


# ==============================================================================
# 7. CUSTOMER RETURNS WIDGET (MODIFIED: Removed Cost Price Display)
# ==============================================================================

# ==============================================================================
# 7. CUSTOMER RETURNS WIDGET (MODIFIED: Cost Removed, Refund Value Displayed)
# ==============================================================================

class CustomerReturnsWidget(QWidget):
    """
    Processes customer returns with a side-by-side view of the original invoice,
    allowing single item returns with automatic calculation of remaining returnable quantity.
    """
    ...
    def __init__(self, db, user_id):
        ...
        self.sale_header_data = None
        self.sale_items_data = None # Fetched raw data
        
        # Set up UI and completers
        self.setLayout(QHBoxLayout())
        self._setup_autocompleter()
        self._setup_ui()
        
    def _setup_autocompleter(self):
        # Product Completer is needed for fast entry (UNCHANGED)
        product_names = self.db.get_product_names()
        self.completer_model = QStringListModel(product_names)
        self.completer = QCompleter(self.completer_model)
        self.completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
    ...
    def _setup_ui(self):
        # --- Left Panel Container (Fixed Minimum Width for Stability) ---
        left_container = QWidget()
        # Set a fixed minimum width to prevent collapse when QMessageBox appears
        left_container.setMinimumWidth(400) 
        left_panel = QVBoxLayout(left_container)
        
        # 1. Title
        title_label = QLabel("Customer Sales Return")
        title_label.setStyleSheet("font-size: 18pt; font-weight: bold; color: #C82333; margin-bottom: 10px;")
        left_panel.addWidget(title_label)
        
        # 2. Invoice Lookup
        lookup_group = QGroupBox("1. Load Original Invoice")
        ...
        self.btn_load_bill = QPushButton("Load Bill Details")
        self.btn_load_bill.clicked.connect(self.load_bill_details)
        self.btn_load_bill.setStyleSheet("background-color: #495057; color: white; padding: 8px;")
        
        lookup_layout.addWidget(QLabel("Invoice/Sale ID:"), 0, 0); lookup_layout.addWidget(self.invoice_input, 0, 1)
        lookup_layout.addWidget(self.btn_load_bill, 1, 0, 1, 2)
        left_panel.addWidget(lookup_group)
        
        # 3. Return Item Details
        return_group = QGroupBox("2. Process Single Item Return")
        self.return_layout = QGridLayout(return_group)
        ...
        self.return_layout.addWidget(QLabel("Product Name:"), 0, 0); self.return_layout.addWidget(self.product_name_input, 0, 1)
        self.return_layout.addWidget(QLabel("Return Quantity:"), 1, 0); self.return_layout.addWidget(self.qty_input, 1, 1)
        self.return_layout.addWidget(QLabel("Reason:"), 2, 0); self.return_layout.addWidget(self.reason_input, 2, 1)
        
        self.process_button = QPushButton("Process Item Return & Restore Stock")
        self.process_button.setStyleSheet("padding: 12px; font-size: 12pt; background-color: #C82333; color: white;")
        self.process_button.clicked.connect(self.process_single_return)
        self.process_button.setEnabled(False)
        
        self.return_layout.addWidget(self.process_button, 3, 0, 1, 2)
        left_panel.addWidget(return_group)
        
        # 4. Result/Refund Display
        result_group = QGroupBox("Return Result (Refund Value)")
        ...
        # Add the left container to the main layout, giving it 1 stretch factor
        self.layout().addWidget(left_container, 1)

        # --- Right Panel: Bill Viewer ---
        right_panel_container = QWidget()
        right_panel = QVBoxLayout(right_panel_container)
        
        self.header_summary_label = QLabel("Bill Summary: No Invoice Loaded")
        self.header_summary_label.setStyleSheet("font-weight: bold; font-size: 12pt; background-color: #F0F8FF; padding: 5px;")
        right_panel.addWidget(self.header_summary_label)
        
        self.bill_items_table = QTableWidget()
        self.bill_items_table.setColumnCount(6)
        self.bill_items_table.setHorizontalHeaderLabels(["Product Name", "Qty Sold", "Unit Price (₹)", "Total Price (₹)", "Qty Remaining", "Batch/Expiry"])
        self.bill_items_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.bill_items_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        self.bill_items_table.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
        self.bill_items_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        self.bill_items_table.cellClicked.connect(self._select_item_from_bill)
        
        right_panel.addWidget(QLabel("Original Bill Items:"))
        right_panel.addWidget(self.bill_items_table)
        
        # Add the right container to the main layout, giving it 2 stretch factors (twice the size of left)
        self.layout().addWidget(right_panel_container, 2)
        
    def _select_item_from_bill(self, row, column):
        """Pre-populates the return form when an item in the bill table is clicked."""
        if not self.sale_items_data: return
        
        selected_product_name = self.bill_items_table.item(row, 0).text()
        
        # Use a case-insensitive search to link the displayed name back to the model
        selected_item = next((item for item in self.sale_items_data if item['name'] == selected_product_name), None)
        
        if selected_item:
            self.product_name_input.setText(selected_item['name'])
            
            # Use the remaining quantity as a default for the return amount, but allow manual edit
            remaining_qty = selected_item['qty_remaining_for_return']
            
            if remaining_qty <= 0.001:
                 QMessageBox.information(self, "No Quantity Remaining", 
                                        f"All {selected_item['name']} from this bill have already been returned.")
                 self.qty_input.clear()
            else:
                self.qty_input.setText(f"{remaining_qty:.2f}")
                self.qty_input.setFocus()
                self.qty_input.selectAll()
        
    def load_bill_details(self):
        """Fetches the sale details, populates the header, and displays items."""
        self.bill_items_table.setRowCount(0)
        self.header_summary_label.setText("Bill Summary: Loading...")
        self.process_button.setEnabled(False)
        self.current_sale_id = None
        self.sale_items_data = None
        ...
        # 1. Fetch Header
        self.sale_header_data = self.db.get_sale_header_data(sale_id)
        if not self.sale_header_data:
            QMessageBox.critical(self, "Error", f"Invoice ID {sale_id} not found.")
            self.header_summary_label.setText(f"Bill Summary: Invoice ID {sale_id} not found.")
            return
            
        if self.sale_header_data['status'] == 'Voided':
            QMessageBox.critical(self, "Error", f"Invoice ID {sale_id} is VOIDED. Stock has been restored.")
            self.header_summary_label.setText(f"Bill Summary: Invoice ID {sale_id} is VOIDED.")
            return

        # 2. Fetch Items
        self.sale_items_data = self.db.get_sale_items_for_display(sale_id)
        if not self.sale_items_data:
            QMessageBox.information(self, "No Items", "No line items found for this invoice.")
            return
            
        self.current_sale_id = sale_id
        self._update_bill_display()
        self.process_button.setEnabled(True)
        self.product_name_input.setFocus()

    def _update_bill_display(self):
        """Updates the right-hand table and header after initial load or a return operation."""
        if not self.sale_header_data or not self.sale_items_data: return

        # Update Header
        self.header_summary_label.setText(
            f"Invoice **#{self.current_sale_id}** ({self.sale_header_data['date']}) | "
            f"Patient: **{self.sale_header_data['patient_name']}** | "
            f"Final Total: **₹{self.sale_header_data['final_total']:.2f}**"
        )

        # Update Items Table
        self.bill_items_table.setRowCount(len(self.sale_items_data))
        for row_num, item in enumerate(self.sale_items_data):
            
            qty_remaining = item['qty_remaining_for_return']
            
            batch_expiry_info = f"Batch: {item['batch_number']} / Exp: {item['expiry_date']}"
            
            data_to_display = [
                item['name'],
                f"{item['qty_sold']:.2f} {item['unit_type']}",
                f"₹{item['unit_price']:.2f}",
                f"₹{item['total_price']:.2f}",
                f"{qty_remaining:.2f} {item['unit_type']}",
                batch_expiry_info
            ]
            
            # Highlight items where quantity remaining for return is 0
            color = QColor(255, 255, 255)
            if qty_remaining <= 0.001:
                color = QColor(200, 255, 200) # Light green means already fully returned
            
            for col_num, value in enumerate(data_to_display):
                item_widget = QTableWidgetItem(value)
                item_widget.setBackground(color)
                
                if col_num in [2, 3, 4]: item_widget.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
                
                self.bill_items_table.setItem(row_num, col_num, item_widget)
        
        self.bill_items_table.resizeColumnsToContents()
        
    def process_single_return(self):
        """Processes one item return, restores stock, and REFRESHES THE BILL DISPLAY."""
        if not self.current_sale_id:
            QMessageBox.warning(self, "Process Error", "Please load an invoice first.")
            return
        ...
        # 1. Perform the core return logic (includes the net quantity check)
        refund_value, message = self.db.record_sales_return_and_restore_stock(
            self.current_sale_id, product_name, qty_to_return, reason, self.user_id
        )
        ...
        if refund_value is not None:
            QMessageBox.information(self, "Return Success",  
                                    ...
                                    f"Stock restored. Refund/Credit calculated: **₹{refund_value:.2f}**")
            
            self.result_label.setText(f"Last Return: Invoice {self.current_sale_id} | Product: {product_name} | Refund: ₹{refund_value:.2f}")
            
            # 2. **IMMEDIATE REFRESH OF BILL DISPLAY** (Stays on the page)
            
            # Clear input for the next item return on the same bill
            self.product_name_input.clear()
            self.qty_input.clear()
            self.product_name_input.setFocus()
            
            # Reload the items data from the DB and update the UI table
            self.sale_items_data = self.db.get_sale_items_for_display(self.current_sale_id)
            self._update_bill_display()
...
# ==============================================================================
# 4. AlertsAndReturnsWidget (FINAL: Uses final widget structure)
# ==============================================================================

# ==============================================================================
# 4. AlertsAndReturnsWidget (UNCHANGED)
# ==============================================================================
...
class AlertsAndReturnsWidget(QTabWidget):
    ...
    def __init__(self, db, user_id):
        ...
        self.addTab(self.supplier_returns_widget, "Supplier Returns (Deduction)")
    ...
    def _handle_tab_change(self, index):
        ...
        if hasattr(tab_widget, 'load_expired_items'):
            ...
            self.low_stock_items_widget.load_low_stock_items() # Use explicit reference since it holds state
        elif hasattr(tab_widget, 'load_returnable_batches'):
            self.supplier_returns_widget.load_returnable_batches() # Use explicit reference since it holds state
...
class NearExpiryAlertsWidget(QWidget):
    ...
    def load_expiry_alerts(self):
        ...
        for row, (status_key, item_str) in enumerate(all_alerts):
            ...
            # Example parsing (UNCHANGED LOGIC)
            try:
...
class LowStockItemsWidget(QWidget):
    ...
    def load_low_stock_items(self):
        """Loads data from the DB, then filters out any items already marked as ordered. (UNCHANGED LOGIC)"""
        
        # 1. Fetch all low stock items from DB (UNCHANGED QUERY)
        query = """
        WITH CurrentStock AS (
            SELECT product_id, SUM(stock_quantity) AS TotalStock
            FROM stock_batches
...
# ==============================================================================
# 6. ALERT WIDGETS - SupplierReturnsWidget (MODIFIED: Removed Pack Cost Display)
# ==============================================================================

# ==============================================================================
# 6. ALERT WIDGETS - SupplierReturnsWidget (MODIFIED: Cost/Refund Removed)
# ==============================================================================
...
class SupplierReturnsWidget(QWidget):
    ...
    # --- UI Setup (Modified Headers) ---
    def _setup_ui(self):
        # Title and Instructions
        title_label = QLabel("Supplier Returns Management (Stock Deductions)") # MODIFIED: Title
        ...
        # MODIFIED HEADER: Pack Cost removed from display header
        ...
        self.available_batches_table.setColumnHidden(6, True) # <<< HIDE PACK COST COLUMN (but data is in model)
        ...
        # --- Bottom Panel: Pending Returns ---
        returns_group = QGroupBox("2. Pending Returns (Stock to be Deducted)") # MODIFIED: Title
        returns_layout = QVBoxLayout(returns_group)
        
        self.pending_returns_table = QTableWidget()
        self.pending_returns_table.setColumnCount(4) # MODIFIED: Reduced to 4 columns
        self.pending_returns_table.setHorizontalHeaderLabels(["Product", "Supplier", "Qty (Packs)", "Reason/Type"]) # MODIFIED: Removed Total Refund
        ...
        # REMOVED: self.total_refund_label
        
        self.process_button = QPushButton("Process All Returns (Deduct Stock)") # MODIFIED: Button text
        ...
        finalize_layout.addStretch(1) # Stretch where refund label used to be
    ...
    def filter_available_batches(self):
        ...
        source_data = self._get_uncommitted_live_batches(raw_batches)
    ...
    # --- Helper to load raw data (MODIFIED: Cost Price Removed from SELECT) ---
    def _load_raw_batches(self):
        ...
        for batch in batches_from_db:
            # Note: T2.pack_cost_price (Index 7 in original) is now missing from query.
            (batch_id, product_id, name, supplier, date_received,
             expiry_date, stock_qty_packs, units_per_pack, unit_type, batch_number) = batch
            
            # Since cost is removed, we hardcode it to 0.0 in the internal model.
            raw_batches.append({
                'batch_id': batch_id, 'product_id': product_id, 'name': name, 'supplier': supplier,  
                'expiry_date': expiry_date, 'stock_qty_packs': stock_qty_packs, 'cost_price': 0.0,  
                'units_per_pack': units_per_pack, 'unit_type': unit_type, 'batch_number': batch_number
            })
        return raw_batches  
        
        
    # --- Helper to apply pending deductions (UNCHANGED LOGIC) ---
    ...
    # --- Modified Loading Method (UNCHANGED LOGIC) ---
    def load_returnable_batches(self):
        ...
        # Update UI text based on state (UNCHANGED LOGIC)
        if self.is_showing_all_stock:
    ...
    def _display_batches(self, batches_to_display):
        ...
        # Re-hide columns correctly
        self.available_batches_table.setColumnHidden(6, True)  
        ...
        for row, batch in enumerate(batches_to_display):
            ...
            # Determine color (UNCHANGED LOGIC)
            if expiry_date < today_yyyymm:
            ...
            # Column 6 (Pack Cost) and 7/8 (Units/Type) are passed but hidden
            data = [
                str(batch['batch_id']), batch['name'], batch['supplier'], batch['batch_number'], expiry_date,  
                f"{stock_qty_packs:.2f} {pack_unit}", f"{batch['cost_price']:.2f}", # cost_price is 0.0 but kept for placeholder column
    ...
    def add_to_pending_returns(self):
        # UNCHANGED LOGIC - NOTE: total_refund is calculated internally based on 0.0 cost price, but not used in UI display.
        ...
        # Hardcoded 0.0 cost price for internal tracking (removed from DB, but needed here to track movement)
        cost_price = 0.0 
        
        if existing_item:
            existing_item['qty_packs'] += qty_to_return
            # existing_item['total_refund'] = existing_item['qty_packs'] * cost_price # REMOVED
        else:
            order_id = None
            self.db.cursor.execute("""
                SELECT T2.order_id FROM stock_batches AS T1
        ...
        # Trigger an update of the displayed table
    ...
    def remove_pending_return(self):
        ...
        if row_index < 0 or row_index >= len(self.pending_returns):
            return
        ...
        # Restore stock to the live_batches_data list for the visible table (UI state restoration)
        restored = False
    ...
    def update_pending_returns_table(self):
        """
        Updates the table showing pending returns.
        MODIFIED: Only displays 4 columns (Product, Supplier, Qty, Reason).
        """
        self.pending_returns_table.setRowCount(len(self.pending_returns))
        
        for row_num, item in enumerate(self.pending_returns):
            
            # NOTE: Total Refund is removed from display.
            data = [
            ...
            for col, value in enumerate(data):
                item_widget = QTableWidgetItem(value)
                if col == 2: item_widget.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
                self.pending_returns_table.setItem(row_num, col, item_widget)
                
        # REMOVED: self.total_refund_label.setText
    ...
    def process_all_returns(self):
        """
        Processes all pending returns, deducting stock and recording the audit trail.
        MODIFIED: Total Refund is no longer part of the confirmation/logging calculation.
        """
        ...
        reply = QMessageBox.question(self, 'Confirm Return Process',
            f"Confirm return of {len(self.pending_returns)} items? \n\nStock will be DEDUCTED and a record created in the returns ledger.",  
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            try:
                for item in items_to_process:
                    # Note: item['cost_price'] is 0.0 but kept as an argument for the DB function.
                    # total_refund_value is hardcoded to 0.0 in the DB layer for this call.
                    success = self.db.record_return_and_deduct_stock(
                        item['product_id'], item['batch_id'], item['qty_packs'],
                        item['supplier'], item['reason'] # Arguments reduced
                    )
                    
                    if success:
                        successful_returns += 1
                        # Safely remove the processed item from pending list
                        self.pending_returns = [p for p in self.pending_returns if p != item]
                    else:
            ...
            if successful_returns > 0:
                ...
                self.db.log_action(
                    ...
                    'RETURNS_PROCESSED',
                    None,
                    f"Processed {successful_returns} supplier returns. Stock deducted." # MODIFIED: Logging simplified
                )
...
# ==============================================================================
# 10. DEDICATED DAILY SALES VIEWER WIDGET (MODIFIED: Currency Symbol Update)
# ==============================================================================

# ==============================================================================
# 10. DEDICATED DAILY SALES VIEWER WIDGET (MODIFIED: Profit/COGS Removed)
# ==============================================================================

class DailySalesViewerWidget(QWidget):
    """Allows viewing all sales and calculating daily revenue for a selected date."""
    ...
    def _setup_ui(self):
        title_label = QLabel("Daily Sales & Revenue Overview")
        title_label.setStyleSheet("font-size: 18pt; font-weight: bold; color: #008080; margin-bottom: 10px;")
        ...
        # --- Summary Card (MODIFIED: Profit removed) ---
        ...
        # --- Table (MODIFIED: Profit column removed) ---
        self.sales_table = QTableWidget()
        self.sales_table.setColumnCount(5) # MODIFIED: Reduced column count
    ...
    def load_daily_sales(self):
        """Loads data using the new DB method and updates the table/summary. (MODIFIED: Uses get_daily_sales_revenue)"""
        selected_date = self.date_selector.date().toString("yyyy-MM-dd")
        self.current_report_data = self.db.get_daily_sales_revenue(selected_date) # MODIFIED: New DB method name
        
        daily_total_revenue = sum(s['revenue'] for s in self.current_report_data)
        
        # Update Summary (MODIFIED: Removed Profit)
        self.summary_label.setText(
            f"Summary for **{selected_date}** ({len(self.current_report_data)} Finalized Sales): "
            f"Total Revenue: **₹{daily_total_revenue:.2f}** | "
            f"Total Discount: **₹{sum(s['discount'] for s in self.current_report_data):.2f}**"
        )

        self._populate_sales_table(self.current_report_data)

    def _filter_sales_table(self):
        """Filters the displayed table based on the search bar content. (UNCHANGED LOGIC)"""
        search_text = self.search_bar.text().strip().lower()
    ...
    def _populate_sales_table(self, data):
        """Populates the sales table with the provided data list. (MODIFIED: Profit column removed)"""
        ...
        for row_num, sale in enumerate(data):
            ...
            # Since profit is removed, coloring based on profit is also removed.
            color = QColor(255, 255, 255) 
            
            data_to_display = [
                sale_id, sale['time'], sale['patient'], sale['doctor'],
                f"{sale['revenue']:.2f}", f"{sale['discount']:.2f}"
            ]
            ...
            for col_num, value in enumerate(data_to_display):
                ...
                if col_num in [0, 4, 5]: # MODIFIED: Indices adjusted
                    item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
    ...
    def _handle_table_double_click(self, index):
        """Opens the SaleDetailDialog when a row is double-clicked. (UNCHANGED)"""
        try:
            ...
            QMessageBox.critical(self, "Error", f"Could not open sale details: {e}")
# ==============================================================================
# 15. DASHBOARD WIDGET (FINAL: Currency Symbol Update)
# ==============================================================================
...
class DashboardWidget(QWidget):
    ...
    def load_dashboard(self):
        ...
        kpi_layout.addWidget(self._create_card(
            "7-Day Revenue", data['last_7_days_revenue'], prefix="₹", style_color="#008080" # Primary Teal
        ))
        # Transactions KPI
        kpi_layout.addWidget(self._create_card(
        ...
        # Inventory Value KPI
        # Displaying Cost Value internally retained, but prefixing with ₹
        kpi_layout.addWidget(self._create_card(
            "Total Inventory Cost Value", data['inventory_value'], prefix="₹", style_color="#495057" # Gray Accent
        ))
...
# ==============================================================================
# 14. MAIN APPLICATION WINDOW (FINAL)
# ==============================================================================

# ==============================================================================
# 14. MAIN APPLICATION WINDOW (FINAL) - MODIFIED FOR F1 SHORTCUT
# ==============================================================================
...
class MainWindow(QMainWindow):
    ...
    # ----------------------------------------------------------------------
    # --- NEW: KEY PRESS EVENT HANDLER FOR F1 SHORTCUT ---
    # ----------------------------------------------------------------------
    def keyPressEvent(self, event):
        """Handle F1 shortcut to open POS screen."""
    ...
    def run_startup_backup(self):
        """Runs the daily backup and updates the status bar on startup. (UNCHANGED)"""
    ...
    def _setup_main_layout(self):
        ...
        self.btn_old_bills = QPushButton("Re-Print/Void Old Bills")
    ...
    def _check_all_alerts(self):
        """Checks for alerts and updates status bar (WITHOUT highlighting nav buttons). (UNCHANGED)"""
        alerts = self.db.get_expiry_alerts()
        data = self.db.get_dashboard_data()
    ...
    def backup_database(self):
        ...
        to that separate location (Manual Backup). (UNCHANGED)
        """
    ...
    def show_inventory_screen(self):
        self.clear_content()
        # MODIFIED: Pass self.user_id
        inventory_widget = InventoryWidget(self.db, self.user_id)  
...
# ==============================================================================
# 9. APPLICATION ENTRY POINT (UNCHANGED)
# ==============================================================================
